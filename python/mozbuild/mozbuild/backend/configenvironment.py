...
class ConfigEnvironment(object):
    ...
    def get_depth(self, file):
        ...
        '''
        return relpath(self.topobjdir, mozpath.dirname(file)).replace(os.sep, '/')
    ...
    def get_input(self, file):
        ...
        '''
        assert(isinstance(file, basestring))
        return mozpath.normpath(mozpath.join(self.topsrcdir, "%s.in" % relpath(file, self.topobjdir)))
    ...
    def create_config_file(self, output, extra=None):
        ...
        '''
        path = output.name
        if mozpath.basename(path) == 'Makefile':
