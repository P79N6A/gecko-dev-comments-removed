	...
	switch( mWindowType ) {
		case eWindowType_popup:
			...
			while( disjoint ) {
				...
				if( PtWidgetIsClass( disjoint, PtWindow ) || PtWidgetIsClass( disjoint, PtServer ) ) break; 
...
NS_IMETHODIMP nsWindow::MakeFullScreen(PRBool aFullScreen)
{
	...
	if( aFullScreen ) {
		...
		PtSetArg( &args[1], Pt_ARG_WINDOW_RENDER_FLAGS, old_render_flags, -1 ); 
