...
nsPrintDialogWidgetGTK::nsPrintDialogWidgetGTK(nsIPrintSettings *aSettings)
{
  ...
  GtkWidget* header_footer_table = gtk_table_new(3, 3, FALSE); 
  ...
  gtk_box_pack_start(GTK_BOX(custom_options_tab), check_buttons_container, FALSE, FALSE, 10); 
...
nsPrintDialogWidgetGTK::GetUTF8FromBundle(const char *aKey)
{
  ...
  return NS_ConvertUTF16toUTF8(intlString);  
}
...
nsPrintDialogWidgetGTK::ImportSettings(nsIPrintSettings *aNSSettings)
{
  ...
  nsCOMPtr<nsIPrintSettingsGTK> aNSSettingsGTK(do_QueryInterface(aNSSettings));
  if (!aNSSettingsGTK)
    return NS_ERROR_FAILURE;

  GtkPrintSettings* settings;
  aNSSettingsGTK->GetGtkPrintSettings(&settings);
  GtkPageSetup* setup;
  aNSSettingsGTK->GetGtkPageSetup(&setup);
...
nsPrintDialogServiceGTK::ShowPageSetup(nsIPrintSettings *aNSSettings)
{
  ...
  GtkPrintSettings* gtkSettings;
  aNSSettingsGTK->GetGtkPrintSettings(&gtkSettings);
  GtkPageSetup* oldPageSetup;
  aNSSettingsGTK->GetGtkPageSetup(&oldPageSetup);
