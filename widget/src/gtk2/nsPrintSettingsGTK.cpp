...
NS_IMETHODIMP
nsPrintSettingsGTK::GetGtkPageSetup(GtkPageSetup **aPageSetup)
{
  NS_ENSURE_ARG_POINTER(aPageSetup);

  *aPageSetup = mPageSetup;
  return NS_OK;
}
...
NS_IMETHODIMP
nsPrintSettingsGTK::GetGtkPrintSettings(GtkPrintSettings **aPrintSettings)
{
...
NS_IMETHODIMP
nsPrintSettingsGTK::GetGtkPrinter(GtkPrinter **aPrinter)
{
  NS_ENSURE_ARG_POINTER(aPrinter);
  
  *aPrinter = mGTKPrinter;
  return NS_OK;
}
...
NS_IMETHODIMP
nsPrintSettingsGTK::GetForcePrintSelectionOnly(PRBool *aPrintSelectionOnly)
{
  NS_ENSURE_ARG_POINTER(aPrintSelectionOnly);

  *aPrintSelectionOnly = mPrintSelectionOnly;
  return NS_OK;
}
...
NS_IMETHODIMP
nsPrintSettingsGTK::SetForcePrintSelectionOnly(PRBool aPrintSelectionOnly)
{
...
nsPrintSettingsGTK::GetToFileName(PRUnichar * *aToFileName)
{
  ...
  nsDependentCString url(gtk_print_settings_get(mPrintSettings, GTK_PRINT_SETTINGS_OUTPUT_URI));
  if (url.IsEmpty()) {
  ...
  NS_GetFileFromURLSpec(url, getter_AddRefs(file));
