...
void TouchInputMapper::timeoutExpired(nsecs_t when) {
    if (mDeviceMode == DEVICE_MODE_POINTER) {
        if (mPointerUsage == POINTER_USAGE_GESTURES) {
            dispatchPointerGestures(when, 0 , true );
        }
    }
}
...
void TouchInputMapper::dispatchPointerUsage(nsecs_t when, uint32_t policyFlags,
    ...
    case POINTER_USAGE_GESTURES:
        dispatchPointerGestures(when, policyFlags, false );
        break;
...
void TouchInputMapper::assignPointerIds() {
    ...
    for (uint32_t i = min(currentPointerCount, lastPointerCount); heapSize > 0 && i > 0; i--) {
        while (heapSize > 0) {
            ...
            if (matchedCurrentBits.hasBit(currentPointerIndex)) continue; 

            uint32_t lastPointerIndex = heap[0].lastPointerIndex;
            if (matchedLastBits.hasBit(lastPointerIndex)) continue; 
...
void MultiTouchInputMapper::configureRawPointerAxes() {
    ...
    if (mRawPointerAxes.trackingId.valid
        ...
        mMultiTouchMotionAccumulator.configure(slotCount, true );
    } else {
        mMultiTouchMotionAccumulator.configure(MAX_POINTERS, false );
    }
}
