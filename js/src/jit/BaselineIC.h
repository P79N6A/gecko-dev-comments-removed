...
class ICIn_Native : public ICInNativeStub
{
  ...
  public:
    static inline ICIn_Native *New(ICStubSpace *space, JitCode *code, HandleShape shape,
                                   HandlePropertyName name)
    {
        if (!code)
            return nullptr;
        return space->allocate<ICIn_Native>(code, shape, name);
    }
};
...
class ICIn_NativePrototype : public ICInNativeStub
{
  ...
  public:
    static inline ICIn_NativePrototype *New(ICStubSpace *space, JitCode *code,
                                            HandleShape shape, HandlePropertyName name,
                                            HandleObject holder, HandleShape holderShape)
    {
        if (!code)
            return nullptr;
        return space->allocate<ICIn_NativePrototype>(code, shape, name, holder, holderShape);
    }
  ...
  public:
    ...
    ICStub *getStub(ICStubSpace *space) {
        ...
        if (kind == ICStub::In_Native) {
            JS_ASSERT(obj_ == holder_);
            return ICIn_Native::New(space, getStubCode(), shape, name_);
        }

        JS_ASSERT(obj_ != holder_);
        JS_ASSERT(kind == ICStub::In_NativePrototype);
        RootedShape holderShape(cx, holder_->lastProperty());
        return ICIn_NativePrototype::New(space, getStubCode(), shape, name_, holder_, holderShape);
    }
};
...
class ICIn_NativeDoesNotExistImpl : public ICIn_NativeDoesNotExist
{
  ...
  public:
    static inline ICIn_NativeDoesNotExistImpl<ProtoChainDepth> *New(
        ICStubSpace *space, JitCode *code, const AutoShapeVector *shapes,
        HandlePropertyName name)
    {
        if (!code)
            return nullptr;
        return space->allocate<ICIn_NativeDoesNotExistImpl<ProtoChainDepth>>(
                    code, shapes, name);
    }

    void traceShapes(JSTracer *trc) {
        for (size_t i = 0; i < NumShapes; i++)
            MarkShape(trc, &shapes_[i], "baseline-innativedoesnotexist-stub-shape");
    }
...
class ICInNativeDoesNotExistCompiler : public ICStubCompiler
{
  ...
  public:
    ...
    ICStub *getStubSpecific(ICStubSpace *space, const AutoShapeVector *shapes) {
        return ICIn_NativeDoesNotExistImpl<ProtoChainDepth>::New(space, getStubCode(),
                                                                 shapes, name_);
    }
...
class ICIn_Dense : public ICStub
{
  ...
  public:
    static inline ICIn_Dense *New(ICStubSpace *space, JitCode *code, HandleShape shape)
    {
        if (!code)
            return nullptr;
        return space->allocate<ICIn_Dense>(code, shape);
    }
    ...
    class Compiler : public ICStubCompiler {
      ...
      public:
        ...
        ICStub *getStub(ICStubSpace *space) {
            return ICIn_Dense::New(space, getStubCode(), shape_);
        }
    };
};
