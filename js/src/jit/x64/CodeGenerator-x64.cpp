...
CodeGeneratorX64::emitSimdLoad(LAsmJSLoadHeap *ins)
{
    ...
    if (numElems == 3) {
        ...
        verifyHeapAccessDisassembly(before, after, true, type, 2, srcAddr,
                                    *ins->output()->output());
        ...
        uint32_t after = masm.size();
        verifyHeapAccessDisassembly(before, after, true, type, numElems, srcAddr, *ins->output()->output());
        masm.append(AsmJSHeapAccess(before, AsmJSHeapAccess::Throw, maybeCmpOffset));
    }
...
CodeGeneratorX64::visitAsmJSLoadHeap(LAsmJSLoadHeap *ins)
{
    ...
    uint32_t after = masm.size();
    verifyHeapAccessDisassembly(before, after, true, accessType, 0, srcAddr, *out->output());
    if (ool) {
        cleanupAfterAsmJSBoundsCheckBranch(mir, ToRegister(ptr));
...
CodeGeneratorX64::emitSimdStore(LAsmJSStoreHeap *ins)
{
    ...
    if (numElems == 3) {
        ...
        after = masm.size();
        verifyHeapAccessDisassembly(before, after, false, type, 2, dstAddr, *ins->value());
        masm.append(AsmJSHeapAccess(before, AsmJSHeapAccess::Throw));
    } else {
        uint32_t before = masm.size();
        storeSimd(type, numElems, in, dstAddr);
        uint32_t after = masm.size();
        verifyHeapAccessDisassembly(before, after, false, type, numElems, dstAddr, *ins->value());
        masm.append(AsmJSHeapAccess(before, AsmJSHeapAccess::Throw, maybeCmpOffset));
    }
...
CodeGeneratorX64::visitAsmJSStoreHeap(LAsmJSStoreHeap *ins)
{
    ...
    uint32_t after = masm.size();
    verifyHeapAccessDisassembly(before, after, false, accessType, 0, dstAddr, *value);
    if (rejoin) {
        cleanupAfterAsmJSBoundsCheckBranch(mir, ToRegister(ptr));
        masm.bind(rejoin);
    }
