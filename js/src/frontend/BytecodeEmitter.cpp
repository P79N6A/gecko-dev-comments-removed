...
{
    const size_t len = 1 + UINT32_INDEX_LEN;
    ptrdiff_t offset = bce->emitCheck(len);
    if (offset < 0)
        return false;
...
EmitPrepareIteratorResult(ExclusiveContext *cx, BytecodeEmitter *bce)
{
    if (bce->script->compileAndGo()) {
        ...
        if (!IteratorResultShape(cx, bce, &shape))
            return false;
        return bce->emitIndex32(JSOP_NEWOBJECT, shape);
    }
...
EmitFinishIteratorResult(ExclusiveContext *cx, BytecodeEmitter *bce, bool done)
{
    ...
    if (!bce->emitIndex32(JSOP_INITPROP, value_id))
        return false;
    if (!bce->emit1(done ? JSOP_TRUE : JSOP_FALSE))
        return false;
    if (!bce->emitIndex32(JSOP_INITPROP, done_id))
        return false;
    return true;
}
...
EmitNameOp(ExclusiveContext *cx, BytecodeEmitter *bce, ParseNode *pn, bool callContext)
{
    ...
    if (op == JSOP_CALLEE) {
        ...
        if (!pn->pn_cookie.isFree()) {
            MOZ_ASSERT(JOF_OPTYPE(op) != JOF_ATOM);
            if (!bce->emitVarOp(pn, op))
                return false;
        } else {
            if (!bce->emitAtomOp(pn, op))
                return false;
        }
    }
...
EmitPropLHS(ExclusiveContext *cx, ParseNode *pn, JSOp op, BytecodeEmitter *bce)
{
    ...
    if (pn2->isKind(PNK_DOT)) {
        ...
        do {
            ...
            if (!bce->emitAtomOp(pndot, JSOP_GETPROP))
                return false;
...
EmitPropIncDec(ExclusiveContext *cx, ParseNode *pn, BytecodeEmitter *bce)
{
    ...
    if (!bce->emit1(JSOP_DUP))                      
        return false;
    if (!bce->emitAtomOp(pn->pn_kid, JSOP_GETPROP)) 
        return false;
    if (!bce->emit1(JSOP_POS))                      
        return false;
    if (post && !bce->emit1(JSOP_DUP))              
        return false;
    ...
    if (post) {
        if (!bce->emit2(JSOP_PICK, (jsbytecode)2))  
            return false;
    ...
    if (!bce->emitAtomOp(pn->pn_kid, setOp))         
        return false;
    if (post && !bce->emit1(JSOP_POP))               
        return false;
...
EmitNameIncDec(ExclusiveContext *cx, ParseNode *pn, BytecodeEmitter *bce)
{
    ...
    if (!bce->emitAtomOp(pn->pn_kid, global ? JSOP_BINDGNAME : JSOP_BINDNAME))  
        return false;
    if (!bce->emitAtomOp(pn->pn_kid, global ? JSOP_GETGNAME : JSOP_GETNAME))    
        return false;
    if (!bce->emit1(JSOP_POS))                      
        return false;
    if (post && !bce->emit1(JSOP_DUP))              
...
EmitElemOperands(ExclusiveContext *cx, ParseNode *pn, JSOp op, BytecodeEmitter *bce)
{
    ...
    if (isSetElem && !bce->emit2(JSOP_PICK, (jsbytecode)2))
        return false;
    return true;
}
...
EmitElemIncDec(ExclusiveContext *cx, ParseNode *pn, BytecodeEmitter *bce)
{
    ...
    if (post) {
        if (!bce->emit2(JSOP_PICK, (jsbytecode)3))  
            return false;
        if (!bce->emit2(JSOP_PICK, (jsbytecode)3))  
            return false;
        if (!bce->emit2(JSOP_PICK, (jsbytecode)2))  
    ...
    if (!EmitElemOpBase(cx, bce, setOp))     
        return false;
    if (post && !bce->emit1(JSOP_POP))       
        return false;
...
EmitNumberOp(ExclusiveContext *cx, double dval, BytecodeEmitter *bce)
{
    ...
    if (NumberIsInt32(dval, &ival)) {
        ...
        if ((int)(int8_t)ival == ival)
            return bce->emit2(JSOP_INT8, (jsbytecode)(int8_t)ival);
    ...
    return bce->emitIndex32(JSOP_DOUBLE, bce->constList.length() - 1);
}
...
#define INTMAP_LENGTH   256
    ...
    if (bce->emitN(switchOp, switchSize) < 0)
        return false;
    ...
    if (switchOp == JSOP_CONDSWITCH) {
        ...
        defaultOffset = bce->emitJump(JSOP_DEFAULT, 0);
        if (defaultOffset < 0)
            return false;
    } else {
        MOZ_ASSERT(switchOp == JSOP_TABLESWITCH);
                 ...
                 jsatomid *result)
{
        ...
        (!bce->sc->isFunctionBox() || bce->sc->asFunctionBox()->isHeavyweight()))
    {
        ...
        if (!UpdateSourceCoordNotes(cx, bce, pn->pn_pos.begin))
            return false;
        if (!bce->emitIndexOp(prologOp, atomIndex))
            return false;
        bce->switchToMain();
    }
        ...
        switch (target->getKind()) {
            ...
            switch (target->getOp()) {
              ...
              case JSOP_SETCONST: {
                ...
                if (!target->isOp(JSOP_SETCONST)) {
                    ...
                    if (!bce->emitIndex32(bindOp, atomIndex))
                        return false;
                    if (!bce->emit1(JSOP_SWAP))
                        return false;
                }

                if (!bce->emitIndexOp(target->getOp(), atomIndex))
                    return false;
                break;
              }
              ...
              case JSOP_INITLEXICAL:
                if (!bce->emitVarOp(target, target->getOp()))
                    return false;
                break;
          ...
          case PNK_DOT:
          {
            ...
            if (!bce->emit1(JSOP_SWAP))
                return false;
            JSOp setOp = bce->sc->strict() ? JSOP_STRICTSETPROP : JSOP_SETPROP;
            if (!bce->emitAtomOp(target, setOp))
                return false;
            break;
          }
...
EmitIteratorNext(ExclusiveContext *cx, BytecodeEmitter *bce, ParseNode *pn=nullptr)
{
    ...
    if (!bce->emit1(JSOP_DUP))                                 
        return false;
    if (!bce->emitAtomOp(cx->names().next, JSOP_CALLPROP))     
        return false;
    if (!bce->emit1(JSOP_SWAP))                                
        return false;
    if (!bce->emitCall(JSOP_CALL, 0, pn))                      
        return false;
...
EmitDefault(ExclusiveContext *cx, BytecodeEmitter *bce, ParseNode *defaultExpr)
{
    ...
    if (!bce->emit1(JSOP_POP))                                 
        return false;
    if (!EmitTree(cx, bce, defaultExpr))                       
        return false;
    ...
    if (op != JSOP_NOP) {
        ...
        switch (lhs->getKind()) {
          case PNK_NAME:
            if (lhs->isConst()) {
                if (lhs->isOp(JSOP_CALLEE)) {
                    ...
                    if (!bce->emitIndex32(lhs->getOp(), atomIndex))
                        return false;
                } else {
                    MOZ_ASSERT(JOF_OPTYPE(lhs->getOp()) != JOF_ATOM);
                    if (!bce->emitVarOp(lhs, lhs->getOp()))
                        return false;
                }
            } else if (lhs->isOp(JSOP_SETNAME) || lhs->isOp(JSOP_STRICTSETNAME)) {
                if (!bce->emit1(JSOP_DUP))
                    return false;
                if (!bce->emitIndex32(JSOP_GETXPROP, atomIndex))
                    return false;
            } else if (lhs->isOp(JSOP_SETGNAME) || lhs->isOp(JSOP_STRICTSETGNAME)) {
                MOZ_ASSERT(lhs->pn_cookie.isFree());
                if (!bce->emitAtomOp(lhs, JSOP_GETGNAME))
                    return false;
            } else if (lhs->isOp(JSOP_SETINTRINSIC)) {
                MOZ_ASSERT(lhs->pn_cookie.isFree());
                if (!bce->emitAtomOp(lhs, JSOP_GETINTRINSIC))
                    return false;
            } else {
                JSOp op;
                switch (lhs->getOp()) {
          ...
          case PNK_DOT: {
            ...
            if (!bce->emitIndex32(isLength ? JSOP_LENGTH : JSOP_GETPROP, atomIndex))
                return false;
            break;
          }
          case PNK_ELEM:
    ...
    switch (lhs->getKind()) {
      case PNK_NAME:
        if (lhs->isOp(JSOP_SETARG) || lhs->isOp(JSOP_SETLOCAL) || lhs->isOp(JSOP_SETALIASEDVAR)) {
            ...
            if (!bce->emitIndexOp(lhs->getOp(), atomIndex))
                return false;
        }
        break;
      case PNK_DOT:
      {
        JSOp setOp = bce->sc->strict() ? JSOP_STRICTSETPROP : JSOP_SETPROP;
        if (!bce->emitIndexOp(setOp, atomIndex))
            return false;
        break;
      }
      case PNK_CALL:
...
EmitIterator(ExclusiveContext *cx, BytecodeEmitter *bce)
{
    ...
    if (!bce->emit1(JSOP_DUP))                                 
        return false;
    if (!bce->emit2(JSOP_SYMBOL, jsbytecode(JS::SymbolCode::iterator))) 
        return false;
    if (!EmitElemOpBase(cx, bce, JSOP_CALLELEM))               
        return false;
    if (!bce->emit1(JSOP_SWAP))                                
        return false;
    if (!bce->emitCall(JSOP_CALL, 0))                          
        return false;
    bce->checkTypeSet(JSOP_CALL);
    return true;
}
...
EmitForOf(ExclusiveContext *cx, BytecodeEmitter *bce, StmtType type, ParseNode *pn, ptrdiff_t top)
{
    ...
    if (type == STMT_FOR_OF_LOOP) {
        ...
        if (!bce->emitDupAt(bce->stackDepth - 1 - 2))          
            return false;
    }
...
EmitYieldStar(ExclusiveContext *cx, BytecodeEmitter *bce, ParseNode *iter, ParseNode *gen)
{
    ...
    if (!bce->emitBackPatchOp(&initialSend))                     
        return false;
    ...
    if (!EmitTree(cx, bce, gen))                                 
        return false;

    
    if (!EmitYieldOp(cx, bce, JSOP_YIELD))                       
        return false;
    ...
    if (!bce->emitBackPatchOp(&subsequentSend))                  
        return false;
    ptrdiff_t tryEnd = bce->offset();                            
    ...
    if (!bce->emit1(JSOP_DUP))                                   
        return false;
    if (!bce->emitAtomOp(cx->names().throw_, JSOP_STRING))       
        return false;
    if (!bce->emit1(JSOP_SWAP))                                  
        return false;
    if (!bce->emit1(JSOP_IN))                                    
        return false;
    
    ptrdiff_t checkThrow = bce->emitJump(JSOP_IFNE, 0);          
    if (checkThrow < 0)
        return false;
    if (!bce->emit1(JSOP_POP))                                   
        return false;
    ...
    if (!bce->emit1(JSOP_DUP))                                   
        return false;
    if (!bce->emitAtomOp(cx->names().throw_, JSOP_CALLPROP))     
        return false;
    if (!bce->emit1(JSOP_SWAP))                                  
        return false;
    if (!bce->emit2(JSOP_PICK, (jsbytecode)3))                   
        return false;
    if (!bce->emitCall(JSOP_CALL, 1, iter))                      
        return false;
    bce->checkTypeSet(JSOP_CALL);
    MOZ_ASSERT(bce->stackDepth == depth);
    ptrdiff_t checkResult = -1;
    if (!bce->emitBackPatchOp(&checkResult))                     
        return false;
    ...
    bce->backPatch(initialSend, bce->code().end(), JSOP_GOTO);  
    bce->backPatch(subsequentSend, bce->code().end(), JSOP_GOTO); 
    ...
    bce->backPatch(checkResult, bce->code().end(), JSOP_GOTO);   
    ...
    if (bce->emitJump(JSOP_IFEQ, tryStart - bce->offset()) < 0)  
        return false;
