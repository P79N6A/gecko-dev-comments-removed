...
BytecodeEmitter::emitFunction(ParseNode* pn, bool needsProto)
{
    ...
    if (fun->isInterpreted()) {
        ...
        if (fun->isInterpretedLazy()) {
            ...
            BytecodeEmitter bce2(this, parser, funbox, script,  js::NullPtr(),
...
BytecodeEmitter::emitYieldStar(ParseNode* iter, ParseNode* gen)
{
    ...
    if (!emitJump(JSOP_IFNE, 0, &checkThrow))                    
        return false;
    if (!emit1(JSOP_POP))                                        
        return false;
    if (!emit1(JSOP_THROW))                                      
    ...
    if (!emitJump(JSOP_IFEQ, tryStart - offset()))               
        return false;
