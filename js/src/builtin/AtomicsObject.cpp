...
js::atomics_futexWakeOrRequeue(JSContext* cx, unsigned argc, Value* vp)
{
    ...
    int32_t woken = 0;
    FutexWaiter whead((uint32_t)-1, nullptr); 
    ...
    FutexWaiter rhead((uint32_t)-1, nullptr); 
...
js::FutexRuntime::lock()
{
 ...
 mozilla::Atomic<PRLock*> FutexRuntime::lock_;

#ifdef DEBUG
 mozilla::Atomic<PRThread*> FutexRuntime::lockHolder_;
#endif
