...
static int allocationNum;               
static int desiredFailure;              
    ...
    int *decpt, int *sign, char **rve, char *buf, size_t bufsize)
{
    ...
    if ((i = (int32)(word0(d) >> Exp_shift1 & (Exp_mask>>Exp_shift1))) != 0) {
        ...
        d2 = x;
        set_word0(d2, word0(d2) - 31*Exp_msk1); 
        i -= (Bias + (P-1) - 1) + 1;
        denorm = 1;
    }
#endif
