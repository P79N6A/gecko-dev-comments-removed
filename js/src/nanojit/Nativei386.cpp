...
#define SIBIDX(n)    "1248"[n]
    ...
    inline void Assembler::JMP_indexed(Register x, I32 ss, NIns** addr) {
        ...
        IMM32(int32_t(addr));
        _nIns -= 3;
        _nIns[0]   = uint8_t(0xff);                         
        _nIns[1]   = uint8_t(0  << 6 | 4 << 3 | 4);         
        _nIns[2]   = uint8_t(ss << 6 | REGNUM(x) << 3 | 5); 
    ...
    void Assembler::asm_arith(LIns* ins)
    {
        ...
        if (!isConstRhs) {
            ...
            case LIR_addi:
            case LIR_addjovi:
            case LIR_addxovi:    ADD(rr, rb); break;     
            case LIR_subi:
            case LIR_subjovi:
    ...
    void Assembler::asm_cmov(LIns* ins)
    {
        ...
        if (!_config.i386_sse2 && ins->isop(LIR_cmovd)) {
            ...
            if (iffalse->isImmD()) {
                asm_immd(FST0, iffalse->immDasQ(), iffalse->immD(), false);
            } else {
