                     ...
                     MDefinition **def, Type *type)
{
    ...
    switch (func) {
      ...
      case AsmJSMathBuiltin_min:    return CheckMathMinMax(f, callNode, def,  false, type);
      case AsmJSMathBuiltin_max:    return CheckMathMinMax(f, callNode, def,  true, type);
...
#undef OP_CHECK_CASE_LIST_
      ...
      case AsmJSSimdOperation_bitselect:
        return CheckSimdSelect(f, call, opType,  false, def, type);
      case AsmJSSimdOperation_select:
        return CheckSimdSelect(f, call, opType,  true, def, type);
...
CheckDivOrMod(FunctionCompiler &f, ParseNode *expr, MDefinition **def, Type *type)
{
    ...
    if (lhsType.isMaybeDouble() && rhsType.isMaybeDouble()) {
        *def = expr->isKind(PNK_DIV)
               ? f.div(lhsDef, rhsDef, MIRType_Double,  false)
               : f.mod(lhsDef, rhsDef, MIRType_Double,  false);
        *type = Type::Double;
        return true;
    }
    ...
    if (lhsType.isMaybeFloat() && rhsType.isMaybeFloat()) {
        if (expr->isKind(PNK_DIV))
            *def = f.div(lhsDef, rhsDef, MIRType_Float32,  false);
        else
    ...
    if (lhsType.isSigned() && rhsType.isSigned()) {
        if (expr->isKind(PNK_DIV))
            *def = f.div(lhsDef, rhsDef, MIRType_Int32,  false);
        else
            *def = f.mod(lhsDef, rhsDef, MIRType_Int32,  false);
        *type = Type::Intish;
        return true;
    }
    ...
    if (lhsType.isUnsigned() && rhsType.isUnsigned()) {
        if (expr->isKind(PNK_DIV))
            *def = f.div(lhsDef, rhsDef, MIRType_Int32,  true);
        else
            *def = f.mod(lhsDef, rhsDef, MIRType_Int32,  true);
        *type = Type::Intish;
        return true;
    }
...
GenerateAsyncInterruptExit(ModuleCompiler &m, Label *throwLabel)
{
    ...
    masm.pushFlags();               
    masm.setFramePushed(0);         
    masm.PushRegsInMask(AllRegsExceptSP, AllRegsExceptSP.fpus()); 
    ...
    masm.PopRegsInMask(RegisterSet(GeneralRegisterSet(0), FloatRegisterSet(FloatRegisters::AllDoubleMask)));   
    masm.mov(r6,sp);
    masm.as_vmsr(r5);
    masm.as_msr(r4);
