      ...
      case TOK_RB:
      {
        ...
        for (JSParseNode *next = pn->pn_head; next; next = next->pn_next) {
            if (PN_TYPE(next) == TOK_COMMA) {
                (void)elts.append(MagicValue(JS_SERIALIZE_NO_NODE)); 
            } else {
                Value expr;
                if (!expression(next, &expr))
                    return false;
                (void)elts.append(expr); 
            }
        }
