	...
	for (i=0; i<kMaxComponents; i++)
	{
		...
		if (FillKeyValueUsingName(currSName, currKey, attrValH, cfgText))
		{
			...
			if (NULL != strstr(*attrValH, attrType))
			{
				...
				gControls->cfg->bAdditionsExist = true; 
			}
			else
...
PopulateMiscKeys(char *cfgText)
{
	...
	{
		section = (char*) malloc((strlen(csection) * sizeof(char)) + 3); 
		if (!section)
			return eParseFailed;
		...
		if (!FillKeyValueSecNameKeyID(rParseKeys, section, sTargetDoc, gControls->cfg->apps[i].targetDoc, cfgText))
			gControls->cfg->apps[i].targetDoc = NULL;	
...
GetNextSection(char **ioTxt, char *outSectionName, char *outSection)
{
	...
	if (*txt == START_SECTION) 
	{
		...
		cnt = 0;
		while (*txt != END_SECTION) 
		{
			if ((*txt==MAC_EOL) || (*txt==WIN_EOL)) 
			{	
				return false;
			}
			
			if( kSNameMaxLen-1 >= cnt++) 
			{
				*snbuf = *txt;
				snbuf++; txt++;
			}
	...
	cnt = 0;
find_contents:
	while (*txt != START_SECTION && *txt != MY_EOF) 
	{
		if( kSectionMaxLen-1 >= cnt++)					
		{
			*sbuf = *txt;
			sbuf++; txt++;
		}
