...
WhackDirectories(char *filename)
{
	...
	{
		prefixLen = tempStr - componentPathStr;		

		strncpy(finalStr, filename, prefixLen);  	
		skipLen = prefixLen + strlen("essential files") + 1;  
	...
	if ((tempStr = strstr(componentPathStr, "components")) != NULL)
	{
		...
		strncpy(finalStr, filename, prefixLen);  	
		skipLen = prefixLen + strlen("components") + 1;  
		strcat(finalStr, filename + skipLen);
		strcpy(filename, finalStr);					
	}
...
InflateFiles(void *hZip, void *hFind, short tgtVRefNum, long tgtDirID)
{
	...
	while (!bFoundAll)
	{
		...
		if (lastslash == (&filename[0] + strlen(filename) - 1)) 
			continue;
		...
		err = GetFullPath(tgtVRefNum, tgtDirID, "\p", &fullPathLen, &fullPathH); 
		if (err!=noErr)
			return err;
		...
		strncat(fullPathStr, *fullPathH, fullPathLen);
		strcat(fullPathStr, macfilename);	
