...
RunAllXPIs(short xpiVRefNum, long xpiDirID, short vRefNum, long dirID)
{
	...
	StringPtr           pTargetSubfolder = "\p"; 
	long                dummyDirID = 0;
	
    err = FSMakeFSSpec(vRefNum, dirID, kViewerFolder, &xpiStubDirSpec); 
    ...
    err = FSMakeFSSpec(gControls->opt->vRefNum, gControls->opt->dirID, 
	...
	if (gControls->tw->allProgressBar)
	{
		SetControlMaximum(gControls->tw->allProgressBar, numXPIs*2); 
			...
			CFragConnectionID* connID, FSSpec& aTargetDir, Str255 errName)
{
	...
	err = GetDiskFragment(&fslib, 0, kCFragGoesToEOF, nil, kReferenceCFrag, connID, &mainAddr, errName);
...
{
	long 	len = *archive;
	char 	*extStart = (char*)(&archive[1] + len - 4); 
