...
LRESULT CALLBACK NewListBoxWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch(uMsg)
  {
    ...
    case WM_LBUTTONDOWN:
      ...
      {
        dwPosX = LOWORD(lParam); 
        dwPosY = HIWORD(lParam); 
...
LPSTR GetStartInstallMessage()
{
  ...
  if(GetPrivateProfileString("Messages", "STR_SETUP_TYPE", "", szBuf, sizeof(szBuf), szFileIniInstall))
    dwBufSize += lstrlen(szBuf) + 2; 
  dwBufSize += 4; 

  switch(dwSetupType)
  {
    case ST_RADIO3:
      dwBufSize += lstrlen(diSetupType.stSetupType3.szDescriptionShort) + 2; 
      break;

    case ST_RADIO2:
      dwBufSize += lstrlen(diSetupType.stSetupType2.szDescriptionShort) + 2; 
      break;

    case ST_RADIO1:
      dwBufSize += lstrlen(diSetupType.stSetupType1.szDescriptionShort) + 2; 
      break;

    default:
      dwBufSize += lstrlen(diSetupType.stSetupType0.szDescriptionShort) + 2; 
      break;
  }
  dwBufSize += 2; 
  ...
  dwBufSize += 4; 
  dwBufSize += lstrlen(sgProduct.szPath) + 2; 
  dwBufSize += 2; 
  ...
  dwBufSize += 4; 
  dwBufSize += lstrlen(sgProduct.szProgramFolderName) + 2; 
  ...
  if(GetTotalArchivesToDownload() > 0)
  {
    ...
    dwBufSize += 4; 
    dwBufSize += lstrlen(szSiteSelectorDescription) + 2; 
  ...
  if((szMessageBuf = NS_GlobalAlloc(dwBufSize)) != NULL)
  {
    ...
    if(GetTotalArchivesToDownload() > 0)
    {
      ...
      lstrcat(szMessageBuf, "    "); 
      lstrcat(szMessageBuf, szSiteSelectorDescription); 
      lstrcat(szMessageBuf, "\r\n");
...
void ResizeAndSetString(HWND aDlg, LPARAM lParam)
{
  ...
  hSTMessage   = GetDlgItem(aDlg, IDC_MESSAGE); 
