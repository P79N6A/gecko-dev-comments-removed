...
PRInt32 nsInstallPatch::Prepare()
{
    ...
    err = NativePatch(  fileName,           
                        mPatchFile,         
                        getter_AddRefs(mPatchedFile));     
...
PRInt32
nsInstallPatch::NativePatch(nsIFile *sourceFile, nsIFile *patchFile, nsIFile **newFile)  
{
  ...
  sourceFile->Clone(getter_AddRefs(outFileSpec));
    ...
    if (( dd->bWin32BoundImage || dd->bMacAppleSingle) && (status == GDIFF_OK ))
    {
        ...
        rv = sourceFile->Clone(getter_AddRefs(tempSrcFile));  
        tempSrcFile->SetLeafName(leafName); 
  ...
  if ( dd->bMacAppleSingle && status == GDIFF_OK ) 
 {
        ...
        outFileSpec->Clone(getter_AddRefs(bsTemp));   
        anotherName = do_QueryInterface(bsTemp, &rv); 
		...
		anotherNameMacSpecific = do_QueryInterface(anotherName, &rv); 
		outSpecMacSpecific = do_QueryInterface(outFileSpec, &rv); 
