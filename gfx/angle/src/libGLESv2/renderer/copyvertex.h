...
{
    const unsigned int rgbSignMask = 0x200;       
    const unsigned int negativeMask = 0xFFFFFC00; 
    ...
    if (toFloat)
    {
        ...
        if (isSigned)
        {
            ...
            if (normalized)
            {
                const int maxValue = 0x1FF;      
                const int minValue = 0xFFFFFE01; 
        ...
        else
        {
            if (normalized)
            {
                const unsigned int maxValue = 0x3FF; 
...
inline void copyPackedVertexData(const void* input, size_t stride, size_t count, void* output)
{
    ...
    const unsigned int rgbMask = 0x3FF; 
    const unsigned int redShift = 0;    
    const unsigned int greenShift = 10; 
    const unsigned int blueShift = 20;  

    const unsigned int alphaMask = 0x3; 
    const unsigned int alphaShift = 30; 
