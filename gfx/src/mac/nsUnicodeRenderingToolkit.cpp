  ...
  if(LoadTransliterator()) {
    ...
    if(NS_SUCCEEDED(mTrans->Convert(tmp.get(), &conv)) && conv) {
      ...
      ::GetPort((GrafPtr*)&thePort);
    	...
    	else if((l>3) && ('v' == conv[0]) && ('(' == conv[1]) && (')' == conv[l-1])) 
    	{
    		short small = aSize * 2 / 3;
    		::TextSize(small);
  			GetScriptTextWidth(conv+2, l-3,oWidth);   
   		...
   		else if((l>1) && ('0' <= conv[0]) && ( conv[0] <= '9') && ('/' == conv[1])) 
    	{
    		short small = aSize * 2 / 3;
    		short tmpw=0;
...
static void CCodeToMacRoman(char aIn, char& aOut,short aWidth, short aHeight, short& oXadj, short& oYadj)
{
	...
	switch(aIn)
	{
		case 'r' : aOut = '\xfb'; oYadj = aHeight * 5 / 6; break ;
		case 'R' : aOut = '\xfb';                          break ;
		case 'd' : aOut = '\xfa'; oYadj = aHeight * 5 / 6; break ;
		case 'D' : aOut = '\xfa';                          break ;
		case 'l' : aOut = '_';                             break ;
		case 'L' : aOut = '\xf8';                          break ;
		case 'c' : aOut = '\xfc';                          break ;
		case 'A' : aOut = '\xab';                          break ;
		case 'x' : aOut = '\xf6'; oYadj= aHeight * 5 / 6;  break ;
		case 'X' : aOut = '\xf6';                          break ;
		case 'G' : aOut = '`';                             break ;
		case 'T' : aOut = '\xf7';                          break ;
		case 't' : aOut = '\xf7'; oYadj= aHeight * 5 / 6;  break ;
		case 'B' : aOut = '\xf9';                          break ;
		case 'b' : aOut = '\xf9'; oYadj= aHeight * 5 / 6;  break ;
		case 'U' : aOut = '\xac';                          break ;
		case 'u' : aOut = '\xac'; oYadj= aHeight * 5 / 6;  break ;
		case 'H' : aOut = ',';    oYadj= - aHeight * 5 / 6;  break ;
		case 'n' : aOut = ',';    oYadj= - aHeight * 5 / 6;  oXadj = aWidth /4; break ;
