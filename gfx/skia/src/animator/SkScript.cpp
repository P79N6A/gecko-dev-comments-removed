...
static const char* errorStrings[] = {
        "array index of out bounds", 
        "could not find reference id", 
        "dot operator expects object", 
        ...
        "expected field name", 
        "expected hex", 
        "expected int for condition operator", 
        "expected number", 
        "expected number for array index", 
        "expected operator", 
        "expected token", 
        "expected token before dot operator", 
        "expected value", 
        "handle member failed", 
        "handle member function failed", 
        "handle unbox failed", 
        "index out of range", 
        "mismatched array brace", 
        "mismatched brackets", 
        "no function handler found", 
        "premature end", 
        "too many parameters", 
...
const SkScriptEngine::SkOperatorAttributes SkScriptEngine::gOpAttributes[] = {
    ...
    { SkOpType(kInt | kScalar | kString), SkOpType(kInt | kScalar | kString), kTowardsString }, 
    ...
    { kInt, kInt, kNoBias }, 
    { SkOpType(kInt | kScalar), SkOpType(kInt | kScalar), kNoBias }, 
    
    { kNoType, kNoType, kNoBias },  
    { kNoType, kNoType, kNoBias },  
    { SkOpType(kInt | kScalar | kString), SkOpType(kInt | kScalar | kString), kTowardsNumber }, 
    ...
    { kNoType, kInt, kNoBias }, 
    { kInt, kInt, kNoBias }, 
    { kNoType, SkOpType(kInt | kScalar), kNoBias }, 
...
scalarCommon:
    ...
    if (suppressed == false && topType != fReturnType &&
            topType == kString && fReturnType != kNoType) { 
        ...
        case kArrayOp:
            ...
            if (match == kParen) 
                ...
                if (indexType != kInt && indexType != kScalar) {
                    fError = kExpectedNumberForArrayIndex; 
                    return -1;
                }
