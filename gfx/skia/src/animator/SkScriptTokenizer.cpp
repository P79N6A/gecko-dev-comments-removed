...
{ SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar | SkOperand2::kString), 
    SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar | SkOperand2::kString), kTowardsString, kResultIsNotBoolean },    
{ SkOperand2::kS32, SkOperand2::kS32, kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::kNoType, SkOperand2::kS32, kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::kS32, SkOperand2::kS32, kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar), 
    SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar), kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar | SkOperand2::kString), 
    SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar |SkOperand2:: kString), kTowardsNumber, 
    kResultIsBoolean }, 
{ SkOperand2::kS32, SkOperand2::kNoType, kNoBias, kResultIsNotBoolean },     
{ SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar | SkOperand2::kString), 
    SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar | SkOperand2::kString), kTowardsNumber,
    kResultIsBoolean }, 
{ SkOperand2::kNoType, SkOperand2::kS32, kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::kNoType, SkOperand2::kS32, kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::kS32, SkOperand2::kS32, kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::kNoType, SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar), kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar), 
    SkOperand2::OpType(SkOperand2::kS32 |SkOperand2:: kScalar), kNoBias, kResultIsNotBoolean }, 
{ SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar), 
    SkOperand2::OpType(SkOperand2::kS32 | SkOperand2::kScalar), kNoBias, kResultIsNotBoolean }, 
...
bool SkScriptEngine2::evaluateDot(const char*& script) { 
    size_t fieldLength = token_length(++script);        
    SkASSERT(fieldLength > 0); 
    ...
    while ((ch = script[0]) != '\0') {
        ...
        if (fBraceStack.count() > baseBrace) {
            if (fBraceStack.top() == kArrayBrace) {
                ...
                success = innerScript(&script, &tokenValue);    
                SkASSERT(success);
                {
...
void SkScriptEngine2::processLogicalOp(Op op) {
    switch (op) {
        case kParen:
        case kArrayOp:
            SkASSERT(fOpStack.count() > 1 && fOpStack.top() == op);    
            if (op == kParen) 
                fOpStack.pop();
            else {
                SkScriptValue2 value;
                fValueStack.pop(&value);
                SkASSERT(value.fType == SkOperand2::kS32 || value.fType == SkOperand2::kScalar); 
                ...
                SkASSERT(arrayValue.fType == SkOperand2::kArray);  
        ...
        case kIf: {
            ...
            SkASSERT(fAccumulatorType != SkOperand2::kString); 
            addToken(kIfOp);
        ...
        case kLogicalOr: {
            ...
            if (fAccumulatorType == SkOperand2::kNoType) {
                SkASSERT(fValueStack.top().fType == SkOperand2::kS32); 
                addTokenValue(fValueStack.top(), kAccumulator);
                fValueStack.pop();
            } else {
...
bool SkScriptEngine2::processOp() {
    ...
    if (constantOperands) {
        ...
        if (attributes->fResultIsBoolean == kResultIsBoolean)
            value1.fType = SkOperand2::kS32;
        else if (attributes->fLeftType == SkOperand2::kNoType) 
