...
const SkMemberInfo* SkDisplayType::GetMembers(SkAnimateMaker* maker, 
    ...
    switch (type) {
        ...
        case SkType_Point: info = Sk_Point::fInfo; infoCount = Sk_Point::fInfoCount; break; 
        CASE_GET_INFO(DrawPoint); 
...
SkDisplayTypes SkDisplayType::GetParent(SkAnimateMaker* maker, SkDisplayTypes base) {
    if (base == SkType_Group || base == SkType_Save || base == SkType_SaveLayer)        
        return SkType_Displayable;
    if (base == SkType_Set)
        return SkType_Animate;  
    const SkMemberInfo* info = GetMembers(maker, base, NULL); 
    SkASSERT(info);
