...
public:
	...
	virtual SkOperand2::OpType getReturnType(int , SkOperand2* operand) { 
		SkDisplayable* displayable = (SkDisplayable*) operand->fObject;
		switch (displayable->getType()) {
			case SkType_Array:
	...
	switch (type) {
		...
		case SkType_Float:
			...
			if (info->fType != SkType_MemberProperty && info->fType != SkType_MemberFunction) 
				value->fS32 = *(int32_t*) info->memberData(displayable);	
			if (type == SkType_MSec) {
				value->fScalar = SkScalarDiv((SkScalar) value->fS32, 1000); 
				type = SkType_Float;
			}
			break;
		case SkType_String: {
		...
		case SkType_Array: {
			SkASSERT(info->fType != SkType_MemberProperty); 
