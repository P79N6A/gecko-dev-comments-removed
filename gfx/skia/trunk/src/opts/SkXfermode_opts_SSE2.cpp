...
static inline __m128i SkDiv255Round_SSE2(const __m128i& a) {
    __m128i prod = _mm_add_epi32(a, _mm_set1_epi32(128)); 
...
static inline __m128i overlay_byte_SSE2(const __m128i& sc, const __m128i& dc,
    ...
    __m128i rc1 = _mm_slli_epi32(sc, 1);                        
    rc1 = Multiply32_SSE2(rc1, dc);                             

    __m128i rc2 = _mm_mullo_epi16(sa, da);                      
    __m128i tmp3 = _mm_slli_epi32(_mm_sub_epi32(da, dc), 1);    
    tmp3 = Multiply32_SSE2(tmp3, _mm_sub_epi32(sa, sc));        
...
static inline __m128i hardlight_byte_SSE2(const __m128i& sc, const __m128i& dc,
    ...
    __m128i rc1 = _mm_mullo_epi16(sc, dc);                
    rc1 = _mm_slli_epi32(rc1, 1);                         
...
static inline __m128i softlight_byte_SSE2(const __m128i& sc, const __m128i& dc,
    ...
    tmp1 = _mm_slli_epi32(sc, 1);                      
    __m128i cmp1 = _mm_cmpgt_epi32(tmp1, sa);
    tmp1 = _mm_sub_epi32(tmp1, sa);                    
    tmp2 = _mm_sub_epi32(_mm_set1_epi32(256), m);      
    ...
    tmp2 = _mm_slli_epi32(dc, 2);                      
    __m128i cmp2 = _mm_cmpgt_epi32(tmp2, da);
    __m128i i = _mm_slli_epi32(m, 2);                  
    __m128i j = _mm_add_epi32(i, _mm_set1_epi32(256)); 
    __m128i k = Multiply32_SSE2(i, j);                 
    __m128i t = _mm_sub_epi32(m, _mm_set1_epi32(256)); 
    i = Multiply32_SSE2(k, t);                         
    i = _mm_srai_epi32(i, 16);                         
    j = Multiply32_SSE2(_mm_set1_epi32(7), m);         
    tmp2 = _mm_add_epi32(i, j);
    i = Multiply32_SSE2(dc, sa);                       
    j = _mm_slli_epi32(sc, 1);                         
    j = _mm_sub_epi32(j, sa);                          
    j = Multiply32_SSE2(da, j);                        
    tmp2 = Multiply32_SSE2(j, tmp2);                   
    tmp2 = _mm_srai_epi32(tmp2, 8);                    
    ...
    tmp3 = Multiply32_SSE2(j, tmp3);                   
    tmp3 = _mm_srai_epi32(tmp3, 8);
    tmp3 = _mm_add_epi32(i, tmp3);                     
    cmp = _mm_and_si128(cmp1, cmp2);
    __m128i rc3 = _mm_and_si128(cmp, tmp3);
    rc = _mm_or_si128(rc, rc3);

    tmp1 = _mm_sub_epi32(_mm_set1_epi32(255), da);     
    tmp1 = _mm_mullo_epi16(sc, tmp1);
    tmp2 = _mm_sub_epi32(_mm_set1_epi32(255), sa);     
...
static inline __m128i exclusion_byte_SSE2(const __m128i& sc, const __m128i& dc,
    ...
    __m128i tmp1 = _mm_mullo_epi16(_mm_set1_epi32(255), sc); 
    __m128i tmp2 = _mm_mullo_epi16(_mm_set1_epi32(255), dc); 
    tmp1 = _mm_add_epi32(tmp1, tmp2);
    tmp2 = _mm_mullo_epi16(sc, dc);                          
    tmp2 = _mm_slli_epi32(tmp2, 1);                          
