                            ...
                            int *nOutputs)
{
       ...
       static WORD CMYKblack[4] = { 0xffff, 0xffff, 0xffff, 0xffff };   
...
WORD Clamp_ab(Fixed32 in)
{
       ...
       if (in < 0) return ENCODE_AB(-128.0);         
       if (in > 0xFFFF) return ENCODE_AB(+127.9961); 
       return (WORD) in;
}
...
{
    int At = (int) floor((double) nEntries * 0.02 + 0.5);   
    double Val, Slope;
    int i;
