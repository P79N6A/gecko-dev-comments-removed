...
HRESULT CUpdateCallbackConsole::ScanProgress(UInt64 , UInt64 , const wchar_t * )
{
  return CheckBreak();
}
...
HRESULT CUpdateCallbackConsole::SetNumFiles(UInt64 )
{
  return S_OK;
}
...
HRESULT CUpdateCallbackConsole::SetRatioInfo(const UInt64 * , const UInt64 * )
{
  if (NConsoleClose::TestBreakSignal())
    return E_ABORT;
  return S_OK;
