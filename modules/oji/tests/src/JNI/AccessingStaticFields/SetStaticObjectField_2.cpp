...
JNI_OJIAPITest(JNIEnv_SetStaticObjectField_2)
{
  GET_JNI_FOR_TEST
  ...
  if((str_chars != NULL) && ((int)len == 3)){
     ...
     return TestResult::PASS("SetStaticObjectField(fieldID incorrect negative) return correct value");
  }else{
     return TestResult::FAIL("SetStaticObjectField(fieldID incorrect negative) return incorrect value!!!!");
  }
