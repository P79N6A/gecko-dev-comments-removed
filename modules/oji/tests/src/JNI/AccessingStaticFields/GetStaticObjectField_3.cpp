...
JNI_OJIAPITest(JNIEnv_GetStaticObjectField_3)
{
  GET_JNI_FOR_TEST
  ...
  if(value == NULL){
     ...
     return TestResult::PASS("GetStaticObjectField(fieldID is incorrect negative) return NULL");
  }else{
     return TestResult::FAIL("GetStaticObjectField(fieldID is incorrect negative) return non-NULL!!!!");
  }
