...
JNI_OJIAPITest(JNIEnv_SetObjectField_1)
{
  GET_JNI_FOR_TEST
  ...
  env->SetObjectField(NULL, fieldID, arr);
  ...
  if((val[0]==0) && (val[1]==0) && (val[2]==0)){
      return TestResult::PASS("SetObjectField(for obj = NULL) passed");
  }else{
      return TestResult::FAIL("SetObjectField(for obj = NULL) failed");
  }
