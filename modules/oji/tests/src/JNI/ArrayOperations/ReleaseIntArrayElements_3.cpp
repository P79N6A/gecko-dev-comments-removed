...
JNI_OJIAPITest(JNIEnv_ReleaseIntArrayElements_3)
{
  GET_JNI_FOR_TEST
  ...
  env->ReleaseIntArrayElements(arr, val, JNI_COMMIT);
  ...
  if((valu[0]==0) && (valu[1]==1) && (valu[2]==2) && (val[0]==0)){
     return TestResult::PASS("ReleaseIntArrayElements(arr, val, JNI_COMMIT) is correct");
  }else{
     return TestResult::FAIL("ReleaseIntArrayElements(arr, val, JNI_COMMIT) is incorrect");
  }
}