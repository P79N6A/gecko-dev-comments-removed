...
JNI_OJIAPITest(JNIEnv_CallNonvirtualIntMethod_29)
{
  GET_JNI_FOR_TEST
  ...
  if(value==0){
     if((excep != NULL) && (env->IsInstanceOf(excep, clazz_exp))){
       ...
       return TestResult::PASS("CallNonvirtualIntMethodA (with class = superclass) for private not inherited method (sig = (ZBCSIJFDLjava/lang/String;[Ljava/lang/String;)I) return correct value");
     }else{
       if(SecENV){
         return TestResult::FAIL("CallNonvirtualIntMethodA (with class = superclass) for private not inherited method (sig = (ZBCSIJFDLjava/lang/String;[Ljava/lang/String;)I) return incorrect value");
       }else{
         return TestResult::PASS("CallNonvirtualIntMethodA (with class = superclass) for private not inherited method (sig = (ZBCSIJFDLjava/lang/String;[Ljava/lang/String;)I) return correct value");
       }
     }
