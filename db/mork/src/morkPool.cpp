...
#include "morkZone.h"
 ...
 void
morkPool::CloseMorkNode(morkEnv* ev) 
{
  if ( this->IsOpenNode() )
  {
    this->MarkClosing();
...
morkRow*
morkPool::NewRow(morkEnv* ev, morkZone* ioZone) 
{
  morkRow* newRow = 0;
  ...
  morkZone* ioZone)
{
  ...
  if ( ev->Good() && fill > inNewSize ) 
  {
    if ( inNewSize ) 
    {
      ...
      if ( newCells )
      {
        morkCell* saveNewCells = newCells; 
        morkCell* oldCells = ioRow->mRow_Cells;
        morkCell* oldEnd = oldCells + fill; 
        morkCell* newEnd = oldCells + inNewSize; 
        ...
        {
          if ( oldCells->mCell_Atom ) 
            oldCells->SetAtom(ev, (morkAtom*) 0, this); 
          ++oldCells;
        }
        oldCells = ioRow->mRow_Cells;
