...
static void rftfsub_128_neon(float* a) {
  ...
  for (j1 = 1, j2 = 2; j2 + 7 < 64; j1 += 4, j2 += 8) {
    ...
    const float32x4_t c_j1 = vld1q_f32(&c[j1]);          
    const float32x4_t c_k1 = vld1q_f32(&c[29 - j1]);     
    const float32x4_t wkrt = vsubq_f32(mm_half, c_k1);   
    const float32x4_t wkr_ = reverse_order_f32x4(wkrt);  
    const float32x4_t wki_ = c_j1;                       
    ...
    const float32x4_t yr_ = vsubq_f32(a_, b_);  
    const float32x4_t yi_ = vaddq_f32(c_, d_);  
...
static void rftbsub_128_neon(float* a) {
  ...
  for (j1 = 1, j2 = 2; j2 + 7 < 64; j1 += 4, j2 += 8) {
    ...
    const float32x4_t c_ = vmulq_f32(wkr_, xi_);
    const float32x4_t d_ = vmulq_f32(wki_, xr_);
    const float32x4_t yr_ = vaddq_f32(a_, b_);  
    const float32x4_t yi_ = vsubq_f32(c_, d_);  
