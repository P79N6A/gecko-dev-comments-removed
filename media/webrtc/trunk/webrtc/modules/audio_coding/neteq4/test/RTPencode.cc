    ...
    case webrtc::kDecoderCELT_32 :
        if (sampfreq==32000) {
            if (enc_frameSize==320) {
                ok=WebRtcCelt_CreateEnc(&CELT32enc_inst[k], 1 );
                if (ok!=0) {
            ...
            ok=WebRtcCelt_EncoderInit(CELT32enc_inst[k],  1 , 48000 );
            if (ok!=0) exit(0);
        } else {
...
#endif
#ifdef CODEC_CELT_32
        else if (coder==webrtc::kDecoderCELT_32) { 
            int encodedLen = 0;
            cdlen = 0;
            while (cdlen <= 0) {
                        ...
                        int seqNo, uint32_t ssrc)
{
    ...
    for(i=0; i<numPayloads-1; i++) {                                            
        if(blockLen[i] > 0) {
            ...
            rtpPointer[0] = (unsigned char) ( 0x80 | (0x7F & payloadType[i]) ); 
            rtpPointer[1] = (unsigned char) ((offset >> 6) & 0xFF);             
            rtpPointer[2] = (unsigned char) ( ((offset & 0x3F)<<2) |
                ( (blockLen[i]>>8) & 0x03 ) );                                  
            rtpPointer[3] = (unsigned char) ( blockLen[i] & 0xFF );             
    ...
    rtpPointer[0]= (unsigned char) (0x00 | (0x7F&payloadType[numPayloads-1]));
    rtpPointer += 1;
