...
void MediaOptTest::Setup(int testType, CmdArgs& args) {
    ...
    if (_testType == 1)
    {
        ...
        else
        {
            if (rateTest == 4000)  
            {
                if (lossTest == 0.1*255) _lossRate = 0.0;  
                else
                    if (lossTest == 0.05*255) _lossRate = 0.1*255;  
                    else
        ...
        if (rateTest == 0.0 || rateTest == 4000) _bitRate = 500; 
        else
...
MediaOptTest::GeneralSetup()
{
    ...
    _vcm->RegisterReceiveCodec(&sendCodec, _numberOfCores); 
