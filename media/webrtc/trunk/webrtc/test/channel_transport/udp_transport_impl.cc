...
#else
int32_t UdpTransportImpl::StartReceiving(uint32_t )
#endif
{
    CriticalSectionScoped cs(_crit);
    if(_receiving)
...
int UdpTransportImpl::SendPacket(int , const void* data, int length)
{
    WEBRTC_TRACE(kTraceStream, kTraceTransport, _id, "%s", __FUNCTION__);
...
int UdpTransportImpl::SendRTCPPacket(int , const void* data,
                                     int length)
{
