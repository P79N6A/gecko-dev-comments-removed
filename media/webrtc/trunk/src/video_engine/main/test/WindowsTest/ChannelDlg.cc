...
BOOL CDXChannelDlg::OnInitDialog()
{
  ...
  if(_parentChannel!=-1)
    m_ctrlDevice.EnableWindow(FALSE); 
...
void CDXChannelDlg::SetCaptureDevice()
{
  ...
  if(captureStr!=_T("N/A") != 0)
  {
        ...
        if(_captureId>=0x1001 && _captureId<0x10FF)
        {
        ...
        else if(_tcsstr(captureStr,_T(".avi"))!=NULL ) 
        {
...
void CDXChannelDlg::OnStopSend()
{
    ...
    TEST_MUSTPASS(_vieBase->StopSend(_channelId),kViEBaseNotSending);   
