...
_height(rhs._height)
{
    ...
    memcpy(_buffer + _startOffset, rhs._buffer, _bufferLength);  
}
...
int TestVideoBuffer::SetOffset(unsigned int length)
{
    ...
    if(oldOffset > length)
    {
        unsigned int newLength = _bufferLength + (oldOffset-length);
    ...
    if(oldOffset < length)
    {
        ...
        {
            _bufferLength -= (length-oldOffset); 
        }
    }
    _startOffset = length; 
...
void TestVideoBuffer::CopyBuffer(TestVideoBuffer& fromVideoBuffer)
{
    ...
    memcpy(_buffer+_startOffset, fromVideoBuffer.GetBuffer(), fromVideoBuffer.GetLength());  
