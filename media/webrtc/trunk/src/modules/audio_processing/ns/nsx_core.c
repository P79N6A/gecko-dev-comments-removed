...
static void UpdateNoiseEstimate(NsxInst_t* inst, int offset) {
  ...
  const WebRtc_Word16 kExp2Const = 11819; 
  ...
  for (i = 0; i < inst->magnLen; i++) {
    ...
    tmp16 -= 21;
    tmp16 += (WebRtc_Word16) inst->qNoise; 
    if (tmp16 < 0) {
...
static void NoiseEstimationC(NsxInst_t* inst,
  ...
  for (i = 0; i < inst->magnLen; i++) {
    noise[i] = (WebRtc_UWord32)(inst->noiseEstQuantile[i]); 
  }
...
static void PrepareSpectrumC(NsxInst_t* inst, int16_t* freq_buf) {
  ...
  for (i = 0; i < inst->magnLen; i++) {
    inst->real[i] = (WebRtc_Word16)WEBRTC_SPL_MUL_16_16_RSFT(inst->real[i],
        (WebRtc_Word16)(inst->noiseSupFilter[i]), 14); 
...
static __inline void CreateComplexBufferC(NsxInst_t* inst,
  ...
  int i = 0, j = 0;
  for (i = 0, j = 0; i < inst->anaLen; i += 1, j += 2) {
    out[j] = WEBRTC_SPL_LSHIFT_W16(in[i], inst->normData); 
    out[j + 1] = 0; 
  }
}
...
void WebRtcNsx_CalcParametricNoiseEstimate(NsxInst_t* inst,
  ...
  if (tmp32no1 > 0) {
    ...
    frac_part = (WebRtc_Word16)(tmp32no1 & 0x000007ff); 
...
void WebRtcNsx_ComputeSpectralFlatness(NsxInst_t* inst, WebRtc_UWord16* magn) {
  ...
  avgSpectralFlatnessDen = inst->sumMagn - (WebRtc_UWord32)magn[0]; 
  ...
  assert(frac < 256);
  tmp32 = (WebRtc_Word32)(((31 - zeros) << 8) + WebRtcNsx_kLogTableFrac[frac]); 
  logCurSpectralFlatness = (WebRtc_Word32)avgSpectralFlatnessNum;
  logCurSpectralFlatness += ((WebRtc_Word32)(inst->stages - 1) << (inst->stages + 7)); 
  ...
  tmp32 = currentSpectralFlatness - (WebRtc_Word32)inst->featureSpecFlat; 
...
void WebRtcNsx_ComputeSpectralDifference(NsxInst_t* inst, WebRtc_UWord16* magnIn) {
  ...
  for (i = 0; i < inst->magnLen; i++) {
    ...
    varMagnUFX += (WebRtc_UWord32)WEBRTC_SPL_MUL_16_16(tmp16no1, tmp16no1); 
    tmp32no1 = WEBRTC_SPL_MUL_32_16(tmp32no2, tmp16no1); 
    covMagnPauseFX += tmp32no1; 
    tmp32no1 = WEBRTC_SPL_RSHIFT_W32(tmp32no2, nShifts); 
    varPauseUFX += (WebRtc_UWord32)WEBRTC_SPL_MUL(tmp32no1, tmp32no1); 
  }
  ...
  if ((varPauseUFX) && (covMagnPauseFX)) {
    tmpU32no1 = (WebRtc_UWord32)WEBRTC_SPL_ABS_W32(covMagnPauseFX); 
...
void WebRtcNsx_SpeechNoiseProb(NsxInst_t* inst, WebRtc_UWord16* nonSpeechProbFinal,
  ...
  for (i = 0; i < inst->magnLen; i++) {
    besselTmpFX32 = (WebRtc_Word32)postLocSnr[i]; 
    ...
    tmp32 = (WebRtc_Word32)(((31 - zeros) << 12) + frac32) - (11 << 12); 
  ...
  inst->featureLogLrt = WEBRTC_SPL_RSHIFT_W32(logLrtTimeAvgKsumFX * 5, inst->stages + 10); 
  ...
  if ((tableIndex < 16) && (tableIndex >= 0)) {
    ...
    frac = (WebRtc_Word16)(tmp32no1 & 0x00003fff); 
  ...
  if (inst->weightSpecFlat) {
    ...
    tmp32no1 = (WebRtc_Word32)WebRtcSpl_DivU32U16(WEBRTC_SPL_LSHIFT_U32(tmpU32no2,
                                                                        nShifts), 25); 
    tmpU32no1 = WebRtcSpl_DivU32U16(WEBRTC_SPL_LSHIFT_U32(tmpU32no2, nShifts), 25); 
    ...
    if (tableIndex < 16) {
      tmp16no2 = kIndicatorTable[tableIndex];
      tmp16no1 = kIndicatorTable[tableIndex + 1] - kIndicatorTable[tableIndex];
      frac = (WebRtc_Word16)(tmpU32no1 & 0x00003fff); 
      tmp16no2 += (WebRtc_Word16)WEBRTC_SPL_MUL_16_16_RSFT(tmp16no1, frac, 14);
      if (tmpIndFX) {
        tmpIndFX = 8192 + tmp16no2; 
      } else {
  ...
  if (inst->priorNonSpeechProb > 0) {
    for (i = 0; i < inst->magnLen; i++) {
      ...
      if (inst->logLrtTimeAvgW32[i] < 65300) {
        ...
        frac = (WebRtc_Word16)(tmp32no1 & 0x00000fff); 
        ...
        if (normTmp + normTmp2 >= 7) {
          ...
          tmp32no1 = WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32)inst->priorNonSpeechProb, 8); 

          nonSpeechProbFinal[i] = (WebRtc_UWord16)WEBRTC_SPL_DIV(tmp32no1,
              (WebRtc_Word32)inst->priorNonSpeechProb + invLrtFX); 
        }
      }
    }
  }
...
void WebRtcNsx_DataAnalysis(NsxInst_t* inst, short* speechFrame, WebRtc_UWord16* magnU16) {
  ...
  magnU16[0] = (WebRtc_UWord16)WEBRTC_SPL_ABS_W16(inst->real[0]); 
  magnU16[inst->anaLen2] = (WebRtc_UWord16)WEBRTC_SPL_ABS_W16(inst->real[inst->anaLen2]);
  inst->sumMagn = (WebRtc_UWord32)magnU16[0]; 
  ...
  if (inst->blockIndex >= END_STARTUP_SHORT) {
    for (i = 1, j = 2; i < inst->anaLen2; i += 1, j += 2) {
      ...
      magnU16[i] = (WebRtc_UWord16)WebRtcSpl_SqrtFloor(tmpU32no1); 
      inst->sumMagn += (WebRtc_UWord32)magnU16[i]; 
    }
  } else {
    ...
    sum_log_magn = (WebRtc_Word32)log2; 
    ...
    sum_log_magn_u16 = (WebRtc_UWord16)WEBRTC_SPL_RSHIFT_W32(tmp_1_w32, zeros);

    
    tmp_2_w32 = WEBRTC_SPL_MUL_16_U16(sum_log_i_square, sum_log_magn_u16); 
    tmpU32no1 = WEBRTC_SPL_RSHIFT_U32((WebRtc_UWord32)sum_log_i_log_magn, 12); 

    
    tmp_u16 = WEBRTC_SPL_LSHIFT_U16((WebRtc_UWord16)sum_log_i, 1); 
    ...
    tmp_2_w32 -= (WebRtc_Word32)WEBRTC_SPL_UMUL_32_16(tmpU32no1, tmp_u16); 
    ...
    tmp_2_w32 += WEBRTC_SPL_LSHIFT_W32((WebRtc_Word32)net_norm, 11); 
    if (tmp_2_w32 < 0) {
      tmp_2_w32 = 0;
    }
...
int WebRtcNsx_ProcessCore(NsxInst_t* inst, short* speechFrame, short* speechFrameHB,
  ...
  for (i = 0; i < inst->magnLen; i++) {
    prevNoiseU16[i] = (WebRtc_UWord16)WEBRTC_SPL_RSHIFT_U32(inst->prevNoiseU32[i], 11); 
  }
  ...
  if (inst->blockIndex < END_STARTUP_SHORT) {
    ...
    if (inst->pinkNoiseExp) {
      pink_noise_exp_avg = (WebRtc_Word16)WebRtcSpl_DivW32W16(inst->pinkNoiseExp,
                                                              (WebRtc_Word16)(inst->blockIndex + 1)); 
      ...
      if (inst->initMagnEst[i]) {
        ...
        if (numerator > tmpU32no1) {
          ...
          noiseSupFilterTmp[i] = (WebRtc_UWord16)WEBRTC_SPL_SAT(16384, tmpU32no2,
              (WebRtc_UWord32)(inst->denoiseBound)); 
        }
      }
  ...
  for (i = 0; i < inst->magnLen; i++) {
    ...
    postLocSnr[i] = 2048; 
    tmpU32no1 = WEBRTC_SPL_LSHIFT_U32((WebRtc_UWord32)magnU16[i], 6); 
    if (postShifts < 0) {
  ...
  for (i = 0; i < inst->magnLen; i++) {
    ...
    if (nonSpeechProbFinal[i] > ONE_MINUS_PROB_RANGE_Q8) {
      if (nShifts < 0) {
        tmp32no1 = (WebRtc_Word32)magnU16[i] - tmp32no2; 
  ...
  for (i = 0; i < inst->magnLen; i++) {
    ...
    if (nShifts < 0) {
      ...
      tmpMagnU32 = (WebRtc_UWord32)magnU16[i]; 
      ...
      tmpMagnU32 = WEBRTC_SPL_LSHIFT_U32((WebRtc_UWord32)magnU16[i], nShifts); 
    ...
    tmpU16no1 = (WebRtc_UWord16)WEBRTC_SPL_UDIV(priorSnr + (tmpU32no1 >> 1), tmpU32no1); 
  ...
  if (inst->fs == 32000) {
    ...
    for (i = inst->anaLen2 - (inst->anaLen2 >> 2); i < inst->anaLen2; i++) {
      ...
      tmpU32no1 += (WebRtc_UWord32)(inst->noiseSupFilter[i]); 
    }
    ...
    if (avgProbSpeechHB < 2048) {
      ...
      gainTimeDomainHB = (WebRtc_Word16)WEBRTC_SPL_MUL_16_16_RSFT(3, avgFilterGainHB, 2); 
      gainTimeDomainHB += gainModHB; 
    }
    
    gainTimeDomainHB
      = WEBRTC_SPL_SAT(16384, gainTimeDomainHB, (WebRtc_Word16)(inst->denoiseBound)); 


    
    for (i = 0; i < inst->blockLen10ms; i++) {
      outFrameHB[i]
        = (WebRtc_Word16)WEBRTC_SPL_MUL_16_16_RSFT(gainTimeDomainHB, inst->dataBufHBFX[i], 14); 
    }
  } 
