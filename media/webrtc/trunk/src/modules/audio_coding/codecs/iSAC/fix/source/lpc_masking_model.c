  ...
  for (k = 0; k < SUBFRAMES; k++) {
    ...
    for (j = 0; j <= ORDERLO; j++)
    {
      for (n = 0; n < j; n++)
      {
        ...
        tmp32 = ((WebRtc_Word32) WEBRTC_SPL_MUL_16_16(a_LOQ11[j], a_LOQ11[n])); 
        
        
        tmp32 = WEBRTC_SPL_LSHIFT_W32(tmp32, 1);
        sh = WebRtcSpl_NormW32(tmp32);
        aSQR32 = WEBRTC_SPL_LSHIFT_W32(tmp32, sh); 
        sh_corr = WebRtcSpl_NormW32(corrlo2QQ[index]);
        tmp32 = WEBRTC_SPL_LSHIFT_W32(corrlo2QQ[index], sh_corr);
        tmp32 = (WebRtc_Word32) WEBRTC_SPL_MUL_32_32_RSFT32BI(aSQR32, tmp32); 
    ...
    for (j = 0; j <= ORDERHI; j++)
    {
      for (n = 0; n < j; n++)
      {
        ...
        tmp32 = ((WebRtc_Word32) WEBRTC_SPL_MUL_16_16(a_HIQ12[j], a_HIQ12[n])); 
        tmp32 = WEBRTC_SPL_LSHIFT_W32(tmp32, 1);
        sh = WebRtcSpl_NormW32(tmp32);
        aSQR32 = WEBRTC_SPL_LSHIFT_W32(tmp32, sh); 
