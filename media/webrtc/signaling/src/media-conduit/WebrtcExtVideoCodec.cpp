...
static const int64_t DEQUEUE_BUFFER_TIMEOUT_US = 100 * 1000ll; 
static const int64_t START_DEQUEUE_BUFFER_TIMEOUT_US = 10 * DEQUEUE_BUFFER_TIMEOUT_US; 
...
static void VideoCodecSettings2AMessage(
  ...
  format->setInt32("bitrate", codecSettings->startBitrate * 1000); 
  ...
  if (isEncoder) {
    format->setInt32("i-frame-interval", std::numeric_limits<int>::max()); 
  }
}
...
int32_t WebrtcExtVideoEncoder::InitEncode(
  ...
  if (!mOmxEncoder) {
    ...
    VideoCodecSettings2AMessage(mOmxEncoder->mMsg, codecSettings, MEDIACODEC_VIDEO_MIME_VP8, true );

    status_t err = mOmxEncoder->Configure(nullptr, nullptr, MediaCodec::CONFIGURE_FLAG_ENCODE, MEDIACODEC_VIDEO_MIME_VP8, true );
    if (err != OK) {
...
int32_t WebrtcExtVideoEncoder::Encode(
  ...
  err = omx->queueInputBuffer(index, 0, size, inputImage.render_time_ms() * PR_USEC_PER_MSEC , 0);
  if (err != OK) {
...
int32_t WebrtcExtVideoEncoder::SetRates(uint32_t newBitRate, uint32_t frameRate) {
  ...
  msg->setInt32("videoBitrate", newBitRate * 1000 );
...
int32_t WebrtcExtVideoDecoder::InitDecode(
  ...
  if (!mOmxDecoder) {
    ...
    VideoCodecSettings2AMessage(mOmxDecoder->mMsg, codecSettings, MEDIACODEC_VIDEO_MIME_VP8, false );

    status_t err = mOmxDecoder->Configure(nullptr, nullptr, 0, MEDIACODEC_VIDEO_MIME_VP8, false );
