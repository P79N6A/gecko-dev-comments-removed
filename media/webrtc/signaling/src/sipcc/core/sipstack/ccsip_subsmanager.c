...
sipSCB_t subsManagerSCBS[MAX_SCBS]; 
...
extern sipPlatformUITimer_t sipPlatformUISMSubNotTimers[]; 
...
static sll_handle_t s_TCB_list = NULL; 
...
subsmanager_handle_ev_app_subscribe (cprBuffer_t buf)
{
    ...
    if (sipSPISendSubscribe(scbp, reSubscribe, FALSE )) {
...
sipSPISendSubNotify (ccsip_common_cb_t *cbp, boolean authen)
{
    ...
    if (cbp->sipCallID[0] == 0) {
        snprintf(tcbp->hb.sipCallID, sizeof(tcbp->hb.sipCallID), "%.8x-%.8x@%s", 
                 (unsigned int) cpr_rand(),
                 (unsigned int) cpr_rand(),
                 src_addr_str);
    }
