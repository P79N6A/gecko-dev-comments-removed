...
sip_header_t sip_cached_headers[] = {
  {"From", "f"},
  {"To", "t"},
  {HTTPISH_HEADER_VIA, "v"},
  {"Call-ID", "i"},
  {"CSeq", NULL},
  {"Contact", "m"},
  {HTTPISH_HEADER_CONTENT_LENGTH, HTTPISH_C_HEADER_CONTENT_LENGTH},
  {HTTPISH_HEADER_CONTENT_TYPE, "c"},
  {"Record-Route", NULL},
  {"Require", NULL},
 {"Route", NULL},
 {"Supported", "k"}
...
sippmh_cmpURLStrings (const char *s1, const char *s2, boolean ignore_case)
{
    ...
    int value1, value2;
    int esc_char_incr_in_s1 = 0; 
    int esc_char_incr_in_s2 = 0; 
        ...
        if (bad_token != NULL) {
            ...
            if (punsupported_tokens && *punsupported_tokens) {
                ...
                if (strlen(*punsupported_tokens) > 0) {
                    sstrncat(*punsupported_tokens, ",", unsupported_tokens_size - strlen(*punsupported_tokens)); 
                }
...

{
    ...
    if (ptr) {
        ptr = ptr + sizeof(SIP_SUBSCRIPTION_STATE_EXPIRES); 
        SKIP_LWS(ptr);
        if (*ptr) {
            boolean is_int = FALSE;
    ...
    if (ptr) {
        ptr = ptr + sizeof(SIP_SUBSCRIPTION_STATE_RETRY_AFTER); 
        SKIP_LWS(ptr);
        if (*ptr) {
            boolean is_int = FALSE;
