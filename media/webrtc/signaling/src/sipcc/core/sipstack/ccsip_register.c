              ...
              [SIPSPI_REG_EV_END - SIPSPI_REG_EV_BASE + 1] =
{
...

    {
      ...
      ccsip_handle_ev_default,
      ccsip_handle_ev_default,
      ccsip_handle_ev_idle_tmr_expire,
      ccsip_handle_ev_default,
      ccsip_handle_ev_default,
...

    {
      ...
      ccsip_handle_ev_3xx,
      ccsip_handle_ev_4xx,
      ccsip_handle_ev_failure_response,
      ccsip_handle_ev_tmr_ack,
...

    {
      ...
      ccsip_handle_ev_default,
      ccsip_handle_ev_standby_keepalive_tmr_expire,
      ccsip_handle_ev_tmr_retry,
      sip_regmgr_ev_default,
     },
        ...
        if (sip_authen) {
            if (sip_authen->scheme == SIP_DIGEST) {
                if (authenticate != NULL) {
                    ...
                    snprintf(uri, sizeof(uri), "sip:%s", ccb->reg.proxy); 
...
ccsip_register_sip2sipreg_event (int sip_event)
{
    ...
    default:
        CCSIP_DEBUG_ERROR("%s: Error: Unknown event.", fname);
...
ccsip_register_send_msg (uint32_t cmd, line_t ndx)
{
    ...
    if (cmd != SIP_REG_UPDATE) {
        ...
        if (ccb != NULL) {
            ...
            if (cc_cfg_table != NULL) {
                ...
                CCSIP_DEBUG_ERROR("%s: Error: cc_cfg_table is null.", fname);
                return SIP_ERROR;
            }
        }
        else {
            CCSIP_DEBUG_ERROR("%s: Error: ccb is null.", fname);
            return SIP_ERROR;
        }
    }
    ...
    if (SIPTaskSendMsg(cmd, register_msg, sizeof(ccsip_registration_msg_t), NULL)
        ...
        CCSIP_DEBUG_ERROR("%s: Error: send buffer failed.", fname);
        return SIP_ERROR;
    }
...
ccsip_register_cancel (boolean cancel_reg, boolean backup_proxy)
{
    ...
    CCSIP_DEBUG_REG_STATE(DEB_F_PREFIX"unregistering %d lines", DEB_F_PREFIX_ARGS(SIP_REG, fname), line_end);
...
ccsip_register_all_lines (void)
{
    ...
    if (value == 0) {
        ...
        CCSIP_DEBUG_REG_STATE(DEB_F_PREFIX"lines already registered", DEB_F_PREFIX_ARGS(SIP_REG, fname));
...
ccsip_register_commit (void)
{
    ...
    case 0:
        if (ccsip_register_get_register_state() != SIP_REG_IDLE) {
            ...
            CCSIP_DEBUG_REG_STATE(DEB_F_PREFIX"lines already unregistered.", DEB_F_PREFIX_ARGS(SIP_REG, fname));
        }
    ...
    default:
        CCSIP_DEBUG_ERROR(DEB_F_PREFIX"Error: invalid register_get= %d", DEB_F_PREFIX_ARGS(SIP_REG, fname),
                          register_get);
    }
}
