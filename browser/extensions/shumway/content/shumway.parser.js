  ...
  (function (SWF) {
    (function (Parser) {
      ...
      function placeObject($bytes, $stream, $, swfVersion, tagCode) {
        ...
        $ || ($ = {});
        if (tagCode > 4 ) {
          flags = $.flags = tagCode > 26  ? Parser.readUi16($bytes, $stream) : Parser.readUi8($bytes, $stream);
      ...
      function defineButton($bytes, $stream, $, swfVersion, tagCode) {
        ...
        if (tagCode == 7) {
          ...
          do {
            ...
            if (swfVersion >= 8) {
              $29.flags = (flags >> 5 & 1 ? 512  : 0) | (flags >> 4 & 1 ? 256  : 0);
            } else {
              $29.flags = 0;
            }
      ...
      function button($bytes, $stream, $, swfVersion, tagCode) {
        ...
        var eob = $.eob = !flags;
        if (swfVersion >= 8) {
          $.flags = (flags >> 5 & 1 ? 512  : 0) | (flags >> 4 & 1 ? 256  : 0);
        } else {
          $.flags = 0;
        }
      ...
      var BodyParser = (function () {
        ...
        BodyParser.prototype.push = function (data, progressInfo) {
          ...
          if (this._initialize) {
            ...
            if (nextTagHeader == ((69  << 6) | FILE_ATTRIBUTES_LENGTH)) {
              stream.ensure(FILE_ATTRIBUTES_LENGTH);
              var substream = stream.substream(stream.pos, stream.pos += FILE_ATTRIBUTES_LENGTH);
              var handler = Parser.tagHandler[69 ];
              var fileAttributesTag = { code: 69  };
              handler(substream.bytes, substream, fileAttributesTag, swfVersion, 69 );
  ...
  (function (SWF) {
    (function (Parser) {
      ...
      function processStyle(style, isLineStyle, dictionary, dependencies) {
        ...
        if (style.type === undefined || style.type === 0 ) {
          return style;
        }
        var scale;
        switch (style.type) {
          ...
          case 66 :
          case 67 :
            style.smooth = style.type !== 66  && style.type !== 67 ;
            style.repeat = style.type !== 65  && style.type !== 67 ;
      ...
      var PathSegment = (function () {
        ...
        PathSegment.prototype._serializeReversed = function (shape, lastPosition) {
          ...
          if (data[dataPosition] !== lastPosition.x || data[dataPosition + 1] !== lastPosition.y) {
            this._writeCommand(9 , dataPosition, data, morphData, shape);
          }
          if (commandsCount === 1) {
        ...
        SegmentedPath.prototype.serialize = function (shape) {
          ...
          if (this.fillStyle) {
            ...
            switch (style.type) {
              ...
              case 18 :
              case 19 :
                var gradientType = style.type === 16  ? 16  : 18 ;
                shape.beginGradient(2 , style.colors, style.ratios, gradientType, style.transform, style.spreadMethod, style.interpolationMode, style.focalPoint | 0);
                break;
              case 65 :
              case 64 :
              case 67 :
              case 66 :
                ...
                shape.beginBitmap(3 , style.bitmapIndex, style.transform, style.repeat, style.smooth);
                break;
              default:
            ...
            switch (style.type) {
              ...
              case 19 :
                var gradientType = style.type === 16  ? 16  : 18 ;
                writeLineStyle(style, shape);
                shape.beginGradient(6 , style.colors, style.ratios, gradientType, style.transform, style.spreadMethod, style.interpolationMode, style.focalPoint | 0);
                break;
              case 65 :
              case 64 :
              case 67 :
              case 66 :
                release || assert(style.bitmapIndex > -1);
                writeLineStyle(style, shape);
                shape.beginBitmap(7 , style.bitmapIndex, style.transform, style.repeat, style.smooth);
                break;
              default:
