...
void HandleConnection(void* data)
{
  ...
  printf("SSLTUNNEL(%x): incoming connection csock(0)=%x, ssock(1)=%x\n", (unsigned int)data, (unsigned int)ci->client_sock, (unsigned int)(PRFileDesc*)other_sock);
  if (other_sock) 
  {
    ...
    while (!((client_error||client_done) && buffers[0].empty() && buffers[1].empty()))
    {
      ...
      printf("SSLTUNNEL(%x): polling flags csock(0)=%c%c, ssock(1)=%c%c\n", (unsigned int)data, 
      ...
      if (pollStatus < 0)
      {
        printf("SSLTUNNEL(%x): pollStatus=%d, exiting\n", (unsigned int)data, pollStatus);
        client_error = true;
        break;
      }
      ...
      if (pollStatus == 0)
      {
        ...
        printf("SSLTUNNEL(%x): poll timeout, looping\n", (unsigned int)data);
        continue;
      }
      ...
      for (PRInt32 s = 0; s < numberOfSockets; ++s)
      {
        ...
        printf("SSLTUNNEL(%x): %csock(%d)=%x out_flags=%d", (unsigned int)data, s==0?'c':'s', s, (unsigned int)sockets[s].fd, out_flags);
  ...
  printf("SSLTUNNEL(%x): exiting root function for csock=%x, ssock=%x\n", (unsigned int)data, (unsigned int)ci->client_sock, (unsigned int)(PRFileDesc*)other_sock);
  if (!client_error)
