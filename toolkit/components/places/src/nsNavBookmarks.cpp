...
nsNavBookmarks::GetDescendantChildren(PRInt64 aFolderId,
  ...
  nsresult rv;
  {
    ...
    while (NS_SUCCEEDED(mDBGetChildren->ExecuteStep(&hasMore)) && hasMore) {
      ...
      PRInt32 itemType;
      rv = mDBGetChildren->GetInt32(kGetChildrenIndex_Type, &itemType);
...
nsNavBookmarks::GetBookmarkURI(PRInt64 aItemId, nsIURI** aURI)
{
  ...
  PRInt32 type;
  rv = mDBGetItemProperties->GetInt32(kGetItemPropertiesIndex_Type, &type);
  NS_ENSURE_SUCCESS(rv, rv);
...
nsNavBookmarks::GetItemType(PRInt64 aItemId, PRUint16* _type)
{
  ...
  PRInt32 itemType;
  rv = mDBGetItemProperties->GetInt32(kGetItemPropertiesIndex_Type, &itemType);
  NS_ENSURE_SUCCESS(rv, rv);
  *_type = itemType;
