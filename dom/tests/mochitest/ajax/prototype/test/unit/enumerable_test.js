...
new Test.Unit.Runner({    
  ...
  testMax: function() {
    ...
    this.assertEqual('sam-', Fixtures.Nicknames.max()); 
  },
  
  testMin: function() {
    this.assertEqual(1, Fixtures.Z.min());
    this.assertEqual(0, [  1, 2, 3, 4, 5, 6, 7, 8, 0, 9 ].min());
    this.assertEqual('Ulysses', Fixtures.Nicknames.min()); 
  },
  ...
  testToArray: function() {
    ...
    this.assert(result != Fixtures.People); 
    this.assertEqual(Fixtures.Nicknames.join(', '),
      result.pluck('nickname').join(', ')); 
  },
