...
test("browser", function() {
	...
	for (var i in browsers) {
		var v = i.toLowerCase().match( /.+(?:rv|it|ra|ie)[\/: ]([\d.]+)/ ); 
		version = v ? v[1] : null;
...
test("attr(String)", function() {
	...
	$('<a id="tAnchor5"></a>').attr('href', '#5').appendTo('#main'); 
...
test("append(String|Element|Array&lt;Element&gt;|jQuery)", function() {
	...
	equals( $("#sap>form").size(), 1, "Check for appending a form" ); 
...
test("$.extend(Object, Object)", function() {
	...
	var ret = jQuery.extend(true, { foo: [] }, { foo: [0] } ); 
