...
const uint32_t kConnectionIdleCheckpointsMS = 2 * 1000; 
...
UpdateRefcountFunction::DidCommit()
{
  ...
  {
    static PLDHashOperator
    Update(const uint64_t& aKey, FileInfoEntry* aValue, void* )
    {
      MOZ_ASSERT(aValue);
...
UpdateRefcountFunction::RollbackSavepoint()
{
  ...
  {
    static PLDHashOperator
    Rollback(const uint64_t& aKey, FileInfoEntry* aValue, void* )
    {
      MOZ_ASSERT(!IsOnBackgroundThread());
      MOZ_ASSERT(aValue);
...
CheckpointConnectionRunnable::Run()
{
  ...
  if (mOwningThread) {
    ...
    mDatabaseInfo->mConnection->Checkpoint( true);
...
OpenDatabaseOp::ActorDestroy(ActorDestroyReason aWhy)
{
  ...
  if (aWhy != Deletion) {
    CleanupBackgroundThreadObjects( true);
  }
          ...
          if (NS_SUCCEEDED(mResultCode)) {
            ...
            if (NS_SUCCEEDED(mResultCode)) {
              ...
              if (mTransaction->GetMode() == IDBTransaction::READ_WRITE_FLUSH &&
                ...
                mResultCode = connection->Checkpoint( false);
              }
