...
#define HEAPMODE 0
  ...
  || (defined(__mips64) && defined(_ABI64)))   
#  define LZ4_ARCH64 1
#else
#  define LZ4_ARCH64 0
#endif
...
{
#if (HEAPMODE)
    void* ctx = ALLOCATOR(LZ4_STREAMSIZE_U32, 4);   
#else
    U32 ctx[LZ4_STREAMSIZE_U32] = {0};      
#endif
    int result;
                 ...
                 )
{
    ...
    while (1)
    {
        ...
        if ((length=(token>>ML_BITS)) == RUN_MASK)
        {
            ...
            if ((sizeof(void*)==4) && unlikely((size_t)(op+length)<(size_t)(op))) goto _output_error;   
            if ((endOnInput) && (sizeof(void*)==4) && unlikely((size_t)(ip+length)<(size_t)(ip))) goto _output_error;   
        }
