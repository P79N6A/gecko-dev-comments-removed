...
#define matchlimit (iend - LASTLITERALS)
...
#ifdef COMPRESS_64K
    if (inputSize>LZ4_64KLIMIT) return 0;   
#endif
#ifdef USE_HEAPMEMORY
    ...
    for ( ; ; )
    {
        ...
        token = op++;
#ifdef LIMITED_OUTPUT
        if unlikely(op + length + (2 + 1 + LASTLITERALS) + (length>>8) > oend) return 0;   
#endif
...
_endCount:
        ...
        length = (int)(ip - anchor);
#ifdef LIMITED_OUTPUT
        if unlikely(op + (1 + LASTLITERALS) + (length>>8) > oend) return 0;    
#endif
