...
struct pt_Continuation
{
    ...
    union { SOCKET osfd; } arg1;            
    union { void* buffer; } arg2;           
    union { PRIntn amount; } arg3;          
...
_PR_MD_PAUSE_CPU(PRIntervalTime ticks)
{
    ...
    if (_nt_idleCount > 0) {
        ...
        while( !PR_CLIST_IS_EMPTY(&_nt_idleList) ) {
            ...
            memset(deadThread, 0xa, sizeof(PRThread)); 
...
static void pt_InsertTimedInternal(pt_Continuation *op)
{
    ...
    if (NULL != pt_tq.op)
    {
        ...
        t_op = pt_tq.op;  
        op_tmo = now + op->timeout;  
        qd_tmo = pt_tq.epoch + t_op->timeout;  
...
static void ContinuationThread(void *arg)
{
    ...
    struct timeval tv;
    SOCKET *pollingList = 0;                
    PRIntn pollingListUsed;                 
    PRIntn pollingListNeeded;               
    PRIntn pollingSlotsAllocated = 0;       
...
static int pt_Continue(pt_Continuation *op)
{
    ...
    pt_InsertTimedInternal(op);  

    PR_NotifyCondVar(pt_tq.new_op);  
