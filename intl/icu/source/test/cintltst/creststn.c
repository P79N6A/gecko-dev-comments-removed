...
static void TestErrorCodes(void) {
  ...
  if(U_SUCCESS(status) && r != NULL) {
    status = U_USING_DEFAULT_WARNING;
    r2 = ures_getByKey(r, "ExemplarCharacters", NULL, &status);  
    checkStatus(__LINE__, U_USING_FALLBACK_WARNING, status);
  }
  ures_close(r);
...
static void TestFallback()
{
    ...
    
    {
        ...
        static const UChar versionStr[] = { 0x0032, 0x002E, 0x0031, 0x002E, 0x0036, 0x002E, 0x0036, 0x0039, 0x0000}; 
...
static void TestGetFunctionalEquivalent(void) {
    ...
    static const char * const collCases[] = {
        ...
        "f",    "zh_TW@collation=pinyin",         "zh", 
        "f",    "zh_Hant_TW@collation=pinyin",    "zh",
        "f",    "zh_CN@collation=stroke",         "zh@collation=stroke", 
