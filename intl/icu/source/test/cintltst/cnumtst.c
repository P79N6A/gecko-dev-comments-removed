...
free(result);
    ...
    {   
        ...
        if (strcmp(parseExpected+2, desta) != 0) {   
            log_err("File %s, Line %d, (expected, actual) = (\"%s\", \"%s\")\n",
...
static void TestSigDigRounding()
{
    ...
    fmt=unum_open(UNUM_DECIMAL, NULL, 0, NULL , NULL, &status);
    if (U_FAILURE(status)) {
        log_data_err("got unexpected error for unum_open: '%s'\n", u_errorName(status));
        return;
    }
...
static const UChar latnDesc[]    = {0x0030,0x0031,0x0032,0x0033,0x0034,0x0035,0x0036,0x0037,0x0038,0x0039,0}; 
static const UChar romanDesc[]   = {0x25,0x72,0x6F,0x6D,0x61,0x6E,0x2D,0x75,0x70,0x70,0x65,0x72,0}; 
...
static UChar currFmtToPatExpected[] = 
