...
void addDateForRgrTest(TestNode** root)
{
    ...
    addTest(root, &Test_GEec,   "tsformat/cdtrgtst/Test_GEec"); 
}
...
void Test4162071() 
{
    ...
    u_uastrcpy(format, "EEE', 'dd-MMM-yyyy HH:mm:ss z"); 
    status = U_ZERO_ERROR;
...
static const UChar zonePST[] = { 0x50,0x53,0x54,0 }; 
...
static const UChar dMyGGGGGPattern[] = { 0x64,0x64,0x20,0x4D,0x4D,0x4D,0x20,0x79,0x79,0x79,0x79,0x20,0x47,0x47,0x47,0x47,0x47,0 }; 
...
static const UChar eeeedMyPattern[]  = { 0x65,0x65,0x65,0x65,0x20,0x64,0x64,0x20,0x4D,0x4D,0x4D,0x20,0x79,0x79,0x79,0x79,0 };      
static const UChar eeeedMyText[]     = { 0x57,0x65,0x64,0x6E,0x65,0x73,0x64,0x61,0x79,0x20,0x30,0x32,0x20,0x4A,0x75,0x6C,0x20,0x32,0x30,0x30,0x38,0 }; 
static const UChar eeeeedMyPattern[] = { 0x65,0x65,0x65,0x65,0x65,0x20,0x64,0x64,0x20,0x4D,0x4D,0x4D,0x20,0x79,0x79,0x79,0x79,0 }; 
...
static const DatePatternAndText datePatternsAndText[] = {
    ...
    { HHmmssPattern,   HHmmssText,   "* HH:mm:ss"        }, 
...
void Test_GEec(void)
{
    ...
    if ( U_SUCCESS(status) ) {
        ...
        for (patTextPtr = datePatternsAndText; patTextPtr->pattern != NULL; ++patTextPtr) {
            ...
            dateResult = udat_parse(dtfmt, patTextPtr->text, -1, NULL, &status); 
            if ( U_FAILURE(status) ) {
