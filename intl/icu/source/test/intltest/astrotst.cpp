...
void AstroTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char*  )
{
...
void AstroTest::closeAstro(UErrorCode &) {
  if(astro != NULL) {
    delete astro;
    astro = NULL;
  }
...
void AstroTest::TestSolarLongitude(void) {
  ...
  for (uint32_t i = 0; i < sizeof(tests)/sizeof(tests[0]); i++) {
    ...
    logln((UnicodeString)"Sun position is " + result.toString() + (UnicodeString)";  "  + " Sun longitude is " + longitude );
  }
  closeAstro(status);
  ASSERT_OK(status);
}
...
void AstroTest::TestLunarPosition(void) {
  ...
  for (int32_t i = 0; i < (int32_t)(sizeof(tests)/sizeof(tests[0])); i++) {
    ...
    logln((UnicodeString)"Moon position is " + result.toString() + (UnicodeString)";  " );
  }
...
void AstroTest::TestCoordinates(void) {
  ...
  logln((UnicodeString)"result is " + result.toString() + (UnicodeString)";  "  );
  closeAstro(status);
  ASSERT_OK(status);
}
...
void AstroTest::TestCoverage(void) {
  ...
  CalendarAstronomer *astronomers[] = {
    myastro, myastro2, myastro3, myastro2 
  };
