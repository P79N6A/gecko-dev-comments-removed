...
void CollationMonkeyTest::TestCollationKey()
{
    if(source.length() == 0) {
    ...
    int32_t result = collationKey1.compareTo(collationKey2);  
    int32_t revResult = collationKey2.compareTo(collationKey1);  
...
void 
CollationMonkeyTest::TestCompare()
{
    if(source.length() == 0) {
    ...
    myCollator->setStrength(Collator::TERTIARY);
    int32_t result = myCollator->compare(subs, subt);  
    int32_t revResult = myCollator->compare(subt, subs);  
    report(subs, subt, result, revResult);
...
void CollationMonkeyTest::TestRules(){
...
void CollationMonkeyTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char*  )
{
    if (exec) logln("TestSuite CollationMonkeyTest: ");
    if(myCollator) {
      switch (index) {
          case 0: name = "TestCompare";   if (exec)   TestCompare(); break;
          case 1: name = "TestCollationKey"; if (exec)    TestCollationKey(); break;
          case 2: name = "TestRules"; if (exec) TestRules(); break;
          default: name = ""; break;
      }
    } else {
