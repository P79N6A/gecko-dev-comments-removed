...
static UBool hasEquivalentTransitions( BasicTimeZone& tz1, BasicTimeZone& tz2,
...
void TimeZoneRuleTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char*  )
{
    if (exec) {
...
TimeZoneRuleTest::TestTimeZoneRuleCoverage(void) {
    ...
    b1 = t3->getPreviousStart(time3, -3*HOUR, 1*HOUR, FALSE, d1); 
    if (b1) {
...
TimeZoneRuleTest::TestSimpleTimeZoneCoverage(void) {
    ...
    stz1->setStartRule(UCAL_MARCH, 11, 2*HOUR, status); 
    stz1->setEndRule(UCAL_NOVEMBER, 1, UCAL_SUNDAY, 2*HOUR, status); 
    if (U_FAILURE(status)) {
    ...
    avail1 = stz1->getNextTransition(time1, FALSE, tzt1); 
    avail2 = stz1->getNextTransition(time2, FALSE, tzt2); 
...
TimeZoneRuleTest::TestT6669(void) {
    ...
    UDate t = 1230681600000.0; 
    UDate expectedNext = 1231027200000.0; 
    UDate expectedPrev = 1215298800000.0; 
...
TimeZoneRuleTest::TestT8943(void) {
    ...
    if (U_FAILURE(status)) {
        ...
        int32_t raw, dst;
        rbtz->getOffset(1293822000000.0 , FALSE, raw, dst, status);
        if (U_FAILURE(status)) {
