...
void DateFormatTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char*  )
{
    if(exec) {
...
void DateFormatTest::TestPatterns() {
    static const struct {
        ...
        {UDAT_MINUTE, "m", "en", "m"},
        {UDAT_HOUR_MINUTE, "jm","en","h:mm a"}, 
        {UDAT_HOUR24_MINUTE, "Hm", "en", "HH:mm"}, 

        {UDAT_SECOND, "s", "en", "s"},
        {UDAT_HOUR_MINUTE_SECOND, "jms","en","h:mm:ss a"}, 
        {UDAT_HOUR24_MINUTE_SECOND, "Hms","en","HH:mm:ss"}, 
        {UDAT_MINUTE_SECOND, "ms", "en", "mm:ss"}, 
...
DateFormatTest::tryPat994(SimpleDateFormat* format, const char* pat, const char* str, UDate expected)
{
    ...
    logln(UnicodeString("Pattern \"") + pat + "\"   String \"" + str + "\"");
        ...
        else
        {
            ...
            if (expected == null ||
                !(date == expected)) errln((UnicodeString)"FAIL: Expected null");
...
void
DateFormatTest::TestLocaleDateFormat() 
{
...
{
    const UDate testDate = 874266720000.;  
    logln((UnicodeString)"Date set to : " + dateToString(testDate));
...
void DateFormatTest::TestRoundtripWithCalendar(void) {
    ...
    DateFormat *formatters[] = {
        DateFormat::createDateTimeInstance(DateFormat::kFull, DateFormat::kFull, Locale("en_US")), 
        DateFormat::createDateTimeInstance(DateFormat::kFull, DateFormat::kFull, Locale("th_TH")), 
...
void DateFormatTest::TestGMTParsing() {
    const char* DATA[] = {
        ...
        "HH:mm:ss zzzz",    "10:20:30 UTC",         "10:20:30 +0000",   
...
void DateFormatTest::TestNumberAsStringParsing()
{
    const NumAsStringItem items[] = {
        ...
        { "ja", TRUE,  FALSE, CharsToUnicodeString("y\\u5E74MMMd\\u65E5"),        CharsToUnicodeString("2009\\u5E747\\u670814\\u65E5")   }, 
        ...
        { "ko", TRUE,  FALSE, UnicodeString("yyyy. MMMMM d."),                    CharsToUnicodeString("2009. 7\\uC6D4 14.")             }, 
        ...
        { "ko", TRUE,  FALSE, CharsToUnicodeString("y\\uB144 MMM d\\uC77C"),      CharsToUnicodeString("2009\\uB144 7\\uC6D4 14\\uC77C") }, 
...
void DateFormatTest::TestMonthPatterns()
{
    ...
    const UnicodeString customPatterns[] = { "y-Ml-d", "G'x'y-Ml-d", "U-M-d", "U MMM d" }; 
...
void DateFormatTest::TestNonGregoFmtParse()
{
    ...
    const TestNonGregoItem items[] = {
        ...
        { NULL, DateFormat::kNone, UnicodeString(""), NULL } 
    };
...
static const UDate TEST_DATE = 1326585600000.;  
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(locales); ++i) {
        ...
        for (DateFormat::EStyle dateStyle = DateFormat::FULL; dateStyle <= DateFormat::SHORT;
            ...
            for (DateFormat::EStyle timeStyle = DateFormat::FULL; timeStyle <= DateFormat::SHORT;
                ...
                if (formattedString.indexOf("n ") >= 0) { 
...
void DateFormatTest::TestNumberFormatOverride() {
    ...
    NumberFormat* check_nf = NumberFormat::createInstance(Locale("en_US"), status);
    assertSuccess("NumberFormat en_US", status);
    fmt->adoptNumberFormat(check_nf); 
    ...
    for(int i=0; i < (int)(sizeof(DATA)/sizeof(DATA[0])); i++){
        ...
        if (fields == (UnicodeString) "") { 
            fmt->adoptNumberFormat(overrideNF);
        } else if (fields == (UnicodeString) "mixed") { 
            ...
            fmt->adoptNumberFormat(overrideNF);
        } else if (fields == (UnicodeString) "Mo"){ 
