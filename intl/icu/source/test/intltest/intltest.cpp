...
static UBool noKnownIssues = FALSE; 
...
void IntlTest::runIndexedTest( int32_t , UBool , const char* & , char*  )
{
...
{
    UnicodeString* strLeak = new UnicodeString("forced leak"); 
...
char *IntlTest::getUnidataPath(char path[]) {
    const int kUnicodeDataTxtLength = 15;  
    ...
    if(f != NULL) {
        fclose(f);
        *(strchr(path, 0) - kUnicodeDataTxtLength) = 0;  
        return path;
    }
