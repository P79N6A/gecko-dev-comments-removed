...
void PluralFormatTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char*  )
{
...
void PluralFormatTest::pluralFormatBasicTest()
{
    UErrorCode status[8];
...
void PluralFormatTest::pluralFormatUnitTest()
{
...
void 
PluralFormatTest::pluralFormatLocaleTest()
{
    int8_t pluralResults[PLURAL_TEST_ARRAY_SIZE];  
    ...
    if (U_FAILURE(status)) {
        ...
        status = U_ZERO_ERROR;
        UnicodeString plResult = plFmt.format(0.0, status);  
        plResult = plFmt.format(0.5, status);  
        plResult = plFmt.format(1.0, status);  
        plResult = plFmt.format(1.9, status);  
        plResult = plFmt.format(2.0, status);  
    }
}
