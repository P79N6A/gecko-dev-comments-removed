...
void CollationTest::runIndexedTest(int32_t index, UBool exec, const char *&name, char * ) {
    if(exec) {
...
void CollationTest::TestShortFCDData() {
    ...
    errorCode.assertSuccess();
    expectedLccc.add(0xdc00, 0xdfff);  
...
void CollationTest::TestFCD() {
    ...
    cpi.resetToStart();
    UCharIterator iter;
    uiter_setString(&iter, s, UPRV_LENGTHOF(s) - 1);  
...
void CollationTest::TestRootElements() {
    ...
    CollationWeights cw1c;  
    CollationWeights cw1u;  
...
void CollationTest::setLocaleCollator(IcuTestErrorCode &errorCode) {
    ...
    delete coll;
    coll = NULL;
    int32_t at = fileLine.indexOf((UChar)0x40, 9);  
    if(at >= 0) {
...
UBool CollationTest::getMergedCollationKey(const UChar *s, int32_t length,
    ...
    for(int32_t i = 0;;) {
        ...
        CollationKey key1(mergedKey.getAlias(), mergedKeyLength);  
        CollationKey key2;
...
void CollationTest::checkCompareStrings(UCHARBUF *f, IcuTestErrorCode &errorCode) {
    ...
    prevString.getTerminatedBuffer();  
    while(readNonEmptyLine(f, errorCode) && !isSectionStarter(fileLine[0])) {
