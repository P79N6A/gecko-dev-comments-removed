...
void IntlTestSpoof::runIndexedTest( int32_t index, UBool exec, const char* &name, char*  )
{
...
void IntlTestSpoof::testSpoofAPI() {
    ...
    TEST_SETUP
        ...
        UnicodeString s2 = UnicodeString("\\u0441\\u0445\\u0455").unescape();  
...
void IntlTestSpoof::testConfData() {
    ...
    while (parseLine.find()) {
        ...
        Normalizer::decompose(rawExpected, FALSE , 0, expected, status);
        TEST_ASSERT_SUCCESS(status);
...
void IntlTestSpoof::testMixedNumbers() {
    ...
    for (int32_t testNum=0; testNum < UPRV_LENGTHOF(tests); testNum++) {
        ...
        status = U_ZERO_ERROR;
        USpoofChecker *sc = uspoof_open(&status);
        uspoof_setChecks(sc, USPOOF_MIXED_NUMBERS, &status); 
        int32_t result = uspoof_checkUnicodeString(sc, testString, NULL, &status);
