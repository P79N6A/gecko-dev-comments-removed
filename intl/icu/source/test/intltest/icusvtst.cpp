...
class ICUNSubclass : public ICUNotifier {
    public:
    UBool acceptsListener(const EventListener& ) const {
        return TRUE;
        
    }

    virtual void notifyListener(EventListener& ) const {
    }
};
...
class LKFSubclass : public LocaleKeyFactory {
    ...
    protected:
    virtual const Hashtable* getSupportedIDs(UErrorCode &) const {
        return &table;
    }
};
...
class TestIntegerService : public ICUService {
    ...
    ICUServiceKey* createKey(const UnicodeString* id, UErrorCode& status) const {
        return LocaleKey::createWithCanonicalFallback(id, NULL, status); 
    }
...
class TestStringService : public ICUService {
  ...
  virtual ICUServiceFactory* createSimpleFactory(UObject* obj, const UnicodeString& id, UBool visible, UErrorCode& ) 
    {
        UnicodeString* s;
...
{
    public:
    virtual UObject* create(const ICUServiceKey& key, const ICUService* , UErrorCode& ) const {
        return new UnicodeString(key.getID());
    }

    virtual void updateVisibleIDs(Hashtable& , UErrorCode& ) const {
        
    }

    virtual UnicodeString& getDisplayName(const UnicodeString& , const Locale& , UnicodeString& result) const {
...
class TestMultipleKeyStringFactory : public ICUServiceFactory {
    ...
    UObject* create(const ICUServiceKey& key, const ICUService* , UErrorCode& status) const {
        if (U_FAILURE(status)) {
        return NULL;
        }
        UnicodeString temp;
...
{
    public:
    static const char* californio; 
    static const char* valley; 
    static const char* surfer; 
    static const char* geek; 
    static Hashtable* supportedIDs; 
...
class WrapFactory : public ICUServiceFactory {
  ...
  static void cleanup() {
    ...
    UnicodeString& getDisplayName(const UnicodeString& id, const Locale& , UnicodeString& result) const {
        result.append("wrap '");
        result.append(id);
        result.append("'");
        return result;
