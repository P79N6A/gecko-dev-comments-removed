...
void RBBITest::testIsBoundary(RuleBasedBreakIterator& bi,  BITestData &td) {
    ...
    for (i = 0; i <= td.fDataToBreak.length(); i++) {
        if (bi.isBoundary(i)) {
            td.fActualBreakPositions.addElement(i, status);  
            tag = bi.getRuleStatus();
            td.fActualTags.addElement(tag, status);
        }
    }
...
static void CharStringAppend(CharString &dest, const UnicodeString &src, UErrorCode &status) {
    ...
    int32_t utf8Length;
    u_strToUTF8WithSub(NULL, 0, &utf8Length,            
...
void RBBITest::executeTest(TestParams *t, UErrorCode &status) {
    ...
    prevBP = utext_nativeLength(t->textToBreak)+2;  
...
UBool RBBITest::testCaseIsKnownIssue(const UnicodeString &testCase, const char *fileName) {
    static const UChar badTestCases[][4] = {                     
        {(UChar)0x200B, (UChar)0x0020, (UChar)0x007D, (UChar)0x0000},   
        {(UChar)0x200B, (UChar)0x0020, (UChar)0x0029, (UChar)0x0000},   
        {(UChar)0x200B, (UChar)0x0020, (UChar)0x0021, (UChar)0x0000},   
        {(UChar)0x200B, (UChar)0x0020, (UChar)0x002c, (UChar)0x0000},   
        {(UChar)0x200B, (UChar)0x0020, (UChar)0x002f, (UChar)0x0000},   
        {(UChar)0x200B, (UChar)0x0020, (UChar)0x2060, (UChar)0x0000}    
    };
...
int32_t RBBILineMonkey::next(int32_t startPos) {
    ...
    for (;;) {
        ...
        if (fNumberMatcher->lookingAt(prevPos, status)) {
            ...
            int32_t numEndIdx = fNumberMatcher->end(status);  
            if (numEndIdx > pos) {
