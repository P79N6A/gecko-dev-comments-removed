...
void NewResourceBundleTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char*  )
{
...
NewResourceBundleTest::TestConstruction()
{
    ...
    char *versionID1 = new char[1 + strlen(U_ICU_VERSION) + strlen(version1)]; 
    char *versionID2 = new char[1 + strlen(U_ICU_VERSION) + strlen(version2)]; 
...
NewResourceBundleTest::TestNewTypes() {
    ...
    if(U_SUCCESS(status)){
        CONFIRM_UErrorCode(status, U_ZERO_ERROR);
        CONFIRM_EQ(uintResult, 0x0FFFFFFF); 
        CONFIRM_EQ(intResult, -1);
