...
void
StringCaseTest::runIndexedTest(int32_t index, UBool exec, const char *&name, char * ) {
    if(exec) {
        ...
        for(whichCase=0; whichCase<TEST_COUNT; ++whichCase) {
            ...
            while(casingTest->nextCase(myCase, status)) {
                ...
                if(whichCase==TEST_TITLE || whichCase==TEST_FOLD) {
                    ...
                    if(optionsString.indexOf((UChar)0x54)>=0) {  
                        options|=U_FOLD_CASE_EXCLUDE_SPECIAL_I;
                    }
                    if(optionsString.indexOf((UChar)0x4c)>=0) {  
                        options|=U_TITLECASE_NO_LOWERCASE;
                    }
                    if(optionsString.indexOf((UChar)0x41)>=0) {  
