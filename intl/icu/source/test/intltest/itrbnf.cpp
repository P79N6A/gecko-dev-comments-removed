...
void IntlTestRBNF::runIndexedTest(int32_t index, UBool exec, const char* &name, char* )
{
...
void IntlTestRBNF::TestLLongSimpleOperators()
{
    ...
    LLAssert((llong(1, 1) <<= 64) == llong(1, 1)); 
    LLAssert((llong(1, 1) <<= -1) == llong(0x80000000, 0)); 
    ...
    LLAssert((llong(0x80000000, 0x80000000) >> 64) == llong(0x80000000, 0x80000000)); 
    LLAssert((llong(0x80000000, 0) >>= -1) == llong(0xffffffff, 0xffffffff)); 
    ...
    LLAssert(llong(0x80000000, 0x80000000).ushr(64) == llong(0x80000000, 0x80000000)); 
    LLAssert(llong(0x80000000, 0).ushr(-1) == llong(0, 1)); 
    ...
    LLAssert(llong(0x80000000, 0) == -llong(0x80000000, 0)); 
    ...
    if (U_FAILURE(status)) {
        ...
        for (int i = 0; testData[i][0]; ++i) {
            ...
            if (U_FAILURE(status)) {
                ...
                FieldPosition pos;
                formatter->format(expectedNumber, actualString, status);
                if (U_FAILURE(status)) {
...
IntlTestRBNF::doLenientParseTest(RuleBasedNumberFormat* formatter, const char* testData[][2]) 
{
    ...
    if (U_FAILURE(status)) {
        ...
        for (int i = 0; testData[i][0]; ++i) {
            const char* spelledNumber = testData[i][0]; 
            const char* asciiUSNumber = testData[i][1]; 
