...
public:
    ...
    virtual UnicodeString& format(    double            , 
                    UnicodeString&        toAppendTo, 
                    FieldPosition&        ) const
    {
        toAppendTo = "";
...
void 
NumberFormatRegressionTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char*  )
{
...
void NumberFormatRegressionTest::Test4087245 (void)
{
    ...
    symbols->setSymbol(DecimalFormatSymbols::kDecimalSeparatorSymbol, UnicodeString((UChar)0x70)); 
    logln(UnicodeString("format(") + n + ") = " +
...
void NumberFormatRegressionTest::Test4086575(void) 
{
    ...
    FieldPosition pos(FieldPosition::DONT_CARE);
    logln("nf: " + nf->format((int32_t)1234, temp, pos)); 
    logln("nf: " + nf->format((int32_t)-1234, temp, pos)); 
...
void NumberFormatRegressionTest::Test4052223(void)
{
    ...
    UErrorCode status = U_ZERO_ERROR;
        ...
        if(! U_FAILURE(status))
            errln(UnicodeString("Bug 4052223 failed : can't parse string \"a\".  Got ") );
...
void NumberFormatRegressionTest::Test4062486(void)
{
    ...
    if (field.getBeginIndex() != 0 && field.getEndIndex() != 5)
        errln(UnicodeString("Format 1234.5 failed. Begin index: ") );
    field.setBeginIndex(7);
    field.setEndIndex(4);
    if (field.getBeginIndex() != 7 && field.getEndIndex() != 4)
        errln("Set begin/end field indexes failed. Begin index: " );
...
void NumberFormatRegressionTest::Test4106662(void)
{
    ...
    if (pos1 == ParsePosition(0)) {
        errln(UnicodeString("Test Failed: \"") + text + "\" is parsed as " );
    }
    delete df;
    df = new DecimalFormat(UnicodeString("$###.00"), status);
    failure(status, "new DecimalFormat");
    df->parse(UnicodeString("$"), num, pos2);
    if (pos2 == ParsePosition(0) ){
        errln(UnicodeString("Test Failed: \"$\" is parsed as ") );
    }
...
void NumberFormatRegressionTest::Test4114639(void)
{
    ...
    if (pos.getErrorIndex() != 8) 
        errln(UnicodeString("Should return null but got : ") ); 
...
void NumberFormatRegressionTest::Test4106667(void)
{
    ...
    if (buffer != UnicodeString("-0")) 
        errln(UnicodeString("  is formatted as ") + buffer);
            ...
            for (int j = 0; j < buf.length(); j++) {
                if (buf[j] == 0x00a4 ) {
                    if(buf[j + 1] == 0x00a4) {
                        ...
                        buf.replace(j, 2, symbols->getSymbol(DecimalFormatSymbols::kIntlCurrencySymbol));
                        j += symbols->getSymbol(DecimalFormatSymbols::kIntlCurrencySymbol).length();
                    }
                    else {
                        buf.replace(j, 1, symbols->getSymbol(DecimalFormatSymbols::kCurrencySymbol)); 
...
void NumberFormatRegressionTest::Test4176114(void) {
    const char* DATA[] = {
        ...
        "00,000", "#00,000",
        "000,000", "#,000,000",
        "0,000,000,000,000.0000", "#0,000,000,000,000.0000", 
    };
...
void NumberFormatRegressionTest::Test8199(void) {
    ...
    UnicodeString numStr = "1000000000.6";   
    Formattable val;
...
void NumberFormatRegressionTest::Test9677(void) {
  ...
  static const UChar negativePrefix[] = { 0x6E,0 }; 
  static const UChar text[] = { 0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0 }; 
  static const UChar text2[] = { 0x6E, 0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0 }; 
