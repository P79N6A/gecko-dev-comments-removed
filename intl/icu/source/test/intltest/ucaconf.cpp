...
void UCAConformanceTest::runIndexedTest( int32_t index, UBool exec, const char* &name, char* )
{
    if(exec) {
...
void UCAConformanceTest::TestTableNonIgnorable() {
    if (U_FAILURE(status)) {
...
void UCAConformanceTest::TestTableShifted() {
    if (U_FAILURE(status)) {
        dataerrln("Error running UCA Conformance Test: %s", u_errorName(status));
        return;
    }
...
void UCAConformanceTest::TestRulesNonIgnorable() {
...
void UCAConformanceTest::TestRulesShifted() {
