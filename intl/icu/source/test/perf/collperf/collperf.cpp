...
struct DataIndex {
    static DWORD        win_langid;     
    static UCollator *  col;            
    uint8_t *   icu_key;
    UChar *     icu_data;
...
public:
    ...
    CollPerfTest(int32_t argc, const char* argv[], UErrorCode& status):UPerfTest(argc, argv, status){
        ...
        UOption options[] = {
            UOPTION_DEF("langid", 'i', UOPT_REQUIRES_ARG),        
                UOPTION_DEF("rulefile", 'r', UOPT_REQUIRES_ARG),      
                
                
                
                UOPTION_DEF("c_french", 'f', UOPT_NO_ARG),          
                UOPTION_DEF("c_alternate", 'a', UOPT_NO_ARG),       
                UOPTION_DEF("c_casefirst", 'c', UOPT_REQUIRES_ARG), 
                UOPTION_DEF("c_caselevel", 'l', UOPT_NO_ARG),       
                UOPTION_DEF("c_normal", 'n', UOPT_NO_ARG),          
                UOPTION_DEF("c_strength", 's', UOPT_REQUIRES_ARG),  
        };
        int32_t opt_len = (sizeof(options)/sizeof(options[0]));
        enum {i, r,f,a,c,l,n,s};   
...
#define TEST_BIN(testname, func)\
    ...
    void prepareData(UErrorCode& status){
        ...
        icu_data_all_len =  icu_data->index[count]; 
        icu_data_all_len -= count;  
        icu_data_all_len += 1;      
        icu_data_all = new UChar[icu_data_all_len];
        icu_data_all[icu_data_all_len - 1] = 0; 
