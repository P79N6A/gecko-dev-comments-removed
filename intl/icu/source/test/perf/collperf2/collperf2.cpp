...
NextSortKeyPart::NextSortKeyPart(const UCollator* coll, const CA_uchar* source, int32_t bufSize, int32_t maxIteration )
    :   coll(coll),
        source(source),
        bufSize(bufSize),
...
NextSortKeyPartUTF8::NextSortKeyPartUTF8(const UCollator* coll, const CA_char* source, int32_t bufSize, int32_t maxIteration )
    :   coll(coll),
        source(source),
        bufSize(bufSize),
        maxIteration(maxIteration),
...
UPerfFunction*
CollPerf2Test::runIndexedTest(int32_t index, UBool exec, const char *&name, char *par )
{
    (void)par;
    TESTCASE_AUTO_BEGIN;
...
UPerfFunction* CollPerf2Test::TestStrcoll()
{
    ...
    Strcoll *testCase = new Strcoll(coll, getData16(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    Strcoll *testCase = new Strcoll(coll, getData16(status), FALSE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    Strcoll_2 *testCase = new Strcoll_2(coll, getData16(status), getModData16(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    StrcollUTF8 *testCase = new StrcollUTF8(coll, getData8(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    StrcollUTF8 *testCase = new StrcollUTF8(coll, getData8(status),FALSE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    StrcollUTF8_2 *testCase = new StrcollUTF8_2(coll, getData8(status), getModData8(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    GetSortKey *testCase = new GetSortKey(coll, getData16(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    GetSortKey *testCase = new GetSortKey(coll, getData16(status), FALSE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPart *testCase = new NextSortKeyPart(coll, getData16(status), 4 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPart *testCase = new NextSortKeyPart(coll, getData16(status), 4 , 4 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPart *testCase = new NextSortKeyPart(coll, getData16(status), 4 , 8 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPart *testCase = new NextSortKeyPart(coll, getData16(status), 32 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPart *testCase = new NextSortKeyPart(coll, getData16(status), 32 , 2 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPartUTF8 *testCase = new NextSortKeyPartUTF8(coll, getData8(status), 4 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPartUTF8 *testCase = new NextSortKeyPartUTF8(coll, getData8(status), 4 , 4 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPartUTF8 *testCase = new NextSortKeyPartUTF8(coll, getData8(status), 4 , 8 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPartUTF8 *testCase = new NextSortKeyPartUTF8(coll, getData8(status), 32 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    NextSortKeyPartUTF8 *testCase = new NextSortKeyPartUTF8(coll, getData8(status), 32 , 2 );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    CppCompare *testCase = new CppCompare(collObj, getData16(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    CppCompare *testCase = new CppCompare(collObj, getData16(status), FALSE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    CppCompare_2 *testCase = new CppCompare_2(collObj, getData16(status), getModData16(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    CppCompareUTF8 *testCase = new CppCompareUTF8(collObj, getData8(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    CppCompareUTF8 *testCase = new CppCompareUTF8(collObj, getData8(status), FALSE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    CppCompareUTF8_2 *testCase = new CppCompareUTF8_2(collObj, getData8(status), getModData8(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    CppGetCollationKey *testCase = new CppGetCollationKey(collObj, getData16(status), TRUE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    CppGetCollationKey *testCase = new CppGetCollationKey(collObj, getData16(status), FALSE );
    if (U_FAILURE(status)) {
        delete testCase;
        return NULL;
    }
