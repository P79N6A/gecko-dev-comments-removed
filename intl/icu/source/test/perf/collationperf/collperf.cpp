...
unsigned long timeGetTime() {
    struct timeval t;
    gettimeofday(&t, 0);
    unsigned long val = t.tv_sec * 1000;  
    val += t.tv_usec / 1000;
    return val;
}
            ...
            for (line=0; line < gNumFileLines; line++) {
                ...
                for (iLoop=0; iLoop < opt_iLoopCount; iLoop++) {
                    t=LCMapStringW(gWinLCID, LCMAP_SORTKEY,
                        ...
                        (unsigned short *)gFileLines[line].winSortKey, 5000);    
                }
            }
        }
    }
...
private:
    UCharFile (const UCharFile & ) {};                         
    UCharFile & operator = (const UCharFile &) {return *this;};   
