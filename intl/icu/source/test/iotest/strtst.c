...
static void TestString(void) {
    ...
    *newDoubleValuePtr = -1.0;
    ...
    u_sprintf(uStringBuf, "UChar %%C: %C", (UChar)0x0041); 
...
static void TestSprintfFormat(void) {
    ...
    const char *reorderFormat = "%2$d==>%1$-10.10s %6$lld %4$-10.10s %3$#x((%5$d"; 
    ...
    TestSPrintFormat("%-8S", abcUChars, "%-8s", abcChars);
    TestSPrintFormat("%.2S", abcUChars, "%.2s", abcChars); 

    TestSPrintFormat("%8s", abcChars, "%8s", abcChars);
    TestSPrintFormat("%-8s", abcChars, "%-8s", abcChars);
    TestSPrintFormat("%.2s", abcChars, "%.2s", abcChars); 
