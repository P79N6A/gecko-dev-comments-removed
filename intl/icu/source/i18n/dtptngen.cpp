...
static const UChar hourFormatChars[] = { CAP_H, LOW_H, CAP_K, LOW_K, 0 }; 
...
DateTimePatternGenerator::addCLDRData(const Locale& locale, UErrorCode& err) {
    ...
    UnicodeString defaultItemFormat(TRUE, UDATPG_ItemFormat, LENGTHOF(UDATPG_ItemFormat)-1);  
    ...
    const char * calendarTypeToUse = DT_DateTimeGregorianTag; 
...
const UnicodeString *
PatternMap::getPatternFromBasePattern(UnicodeString& basePattern, UBool& skeletonWasSpecified) { 
   PtnElem *curElem;
...
const UnicodeString *
PatternMap::getPatternFromSkeleton(PtnSkeleton& skeleton, const PtnSkeleton** specifiedSkeletonPtr) { 
   PtnElem *curElem;
