...
static const ucolTokOption rulesOptions[UTOK_OPTION_COUNT] = {
     {option_02,  9, alternateSub, 2, UCOL_ALTERNATE_HANDLING}, 
     {option_03,  9, frenchSub, 1, UCOL_FRENCH_COLLATION}, 
     {option_07,  9, onOffSub, 2, UCOL_CASE_LEVEL},  
     {option_08,  9, caseFirstSub, 3, UCOL_CASE_FIRST}, 
     {option_06, 13, onOffSub, 2, UCOL_NORMALIZATION_MODE}, 
     {option_13, 9, onOffSub, 2, UCOL_HIRAGANA_QUATERNARY_MODE}, 
     {option_14, 8, strengthSub, 5, UCOL_STRENGTH}, 
     {option_19, 15, onOffSub, 2, UCOL_NUMERIC_COLLATION},  
     {option_04, 12, NULL, 0, UCOL_ATTRIBUTE_COUNT}, 
     {option_01,  9, NULL, 0, UCOL_ATTRIBUTE_COUNT}, 
     {option_12,  6, beforeSub, 3, UCOL_ATTRIBUTE_COUNT}, 
     {option_05,  3, NULL, 0, UCOL_ATTRIBUTE_COUNT}, 
     {option_15,  5, firstLastSub, 7, UCOL_ATTRIBUTE_COUNT}, 
     {option_16,  4, firstLastSub, 7, UCOL_ATTRIBUTE_COUNT}, 
     {option_17,  8, NULL, 0, UCOL_ATTRIBUTE_COUNT}, 
     {option_18, 20, NULL, 0, UCOL_ATTRIBUTE_COUNT}, 
     {option_00,  9, NULL, 0, UCOL_ATTRIBUTE_COUNT}, 
     {option_09, 11, NULL, 0, UCOL_ATTRIBUTE_COUNT}, 
     {option_10, 11, NULL, 0, UCOL_ATTRIBUTE_COUNT}, 
     {option_11,  7, NULL, 0, UCOL_ATTRIBUTE_COUNT},  
     {option_20,  6, NULL, 0, UCOL_ATTRIBUTE_COUNT},  
     {option_21,  7, NULL, 0, UCOL_ATTRIBUTE_COUNT}  
};
                         ...
                         UErrorCode *status)
{
    ...
    while(start < end && PatternProps::isWhiteSpace(*start)) { 
        start++;
    }
    if(start >= end) {
        return NULL;
    ...
    while(i < UTOK_OPTION_COUNT) {
        if(u_strncmpNoCase(start, rulesOptions[i].optionName, rulesOptions[i].optionLen) == 0) {
            ...
            if(end - start > rulesOptions[i].optionLen) {
                optionArg = start+rulesOptions[i].optionLen+1; 
                while(PatternProps::isWhiteSpace(*optionArg)) { 
                    optionArg++;
                }
            }
            break;
...
int32_t ucol_uprv_tok_readOption(const UChar *start, const UChar *end, const UChar **optionArg) {
    ...
    while(PatternProps::isWhiteSpace(*start)) { 
        start++;
    }
    while(i < UTOK_OPTION_COUNT) {
        if(u_strncmpNoCase(start, rulesOptions[i].optionName, rulesOptions[i].optionLen) == 0) {
            if(end - start > rulesOptions[i].optionLen) {
                *optionArg = start+rulesOptions[i].optionLen; 
                while(PatternProps::isWhiteSpace(**optionArg)) { 
                    (*optionArg)++;
                }
            }
...
inline UColToken *getVirginBefore(UColTokenParser *src, UColToken *sourceToken, uint8_t strength, UParseError *parseError, UErrorCode *status) {
    ...
    if(sourceToken != NULL) {
        ...
        uprv_init_collIterate(src->UCA, src->source+src->parsedToken.charsOffset , 1, &s, status);
    }
    if(U_FAILURE(*status)) {
        return NULL;
    }
    ...
    if((baseCE & 0xFF000000) >= (consts->UCA_PRIMARY_IMPLICIT_MIN<<24) && (baseCE & 0xFF000000) <= (consts->UCA_PRIMARY_IMPLICIT_MAX<<24) ) { 
        ...
        if(U_SUCCESS(*status) && parseEnd != NULL) {
            ...
            if(src->parsedToken.strength != UCOL_TOK_RESET) {
                ...
                if(lastStrength == UCOL_TOK_RESET
                    ...
                    if(expandNext != 0) {
                        if(sourceToken->strength == UCOL_PRIMARY) { 
                            expandNext = 0;
                        } else if(sourceToken->expansion == 0) { 
                ...
                if((specs & UCOL_TOK_BEFORE) != 0) { 
                    if(top == FALSE) { 
                        ...
                        uint32_t baseContCE = ucolIndirectBoundaries[src->parsedToken.indirectIndex].startContCE;
                        uint32_t CE = UCOL_NOT_FOUND, SecondCE = UCOL_NOT_FOUND;

                        UCAConstants *consts = (UCAConstants *)((uint8_t *)src->UCA->image + src->UCA->image->UCAConsts);
                        if((baseCE & 0xFF000000) >= (consts->UCA_PRIMARY_IMPLICIT_MIN<<24) && 
                           (baseCE & 0xFF000000) <= (consts->UCA_PRIMARY_IMPLICIT_MAX<<24) ) { 
                            uint32_t primary = (baseCE & UCOL_PRIMARYMASK) | ((baseContCE & UCOL_PRIMARYMASK) >> 16);
                            uint32_t raw = uprv_uca_getRawFromImplicit(primary);
                            uint32_t primaryCE = uprv_uca_getImplicitFromRaw(raw-1);
                            CE = (primaryCE & UCOL_PRIMARYMASK) | 0x0505;
...
void ucol_tok_initTokenList(
    ...
    src->extraCurrent = src->end+1; 
    src->extraEnd = src->source+estimatedSize; 
    src->varTop = NULL;
    src->UCA = UCA;
