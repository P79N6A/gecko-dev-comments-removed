...
uint32_t
UIterCollationIterator::handleNextCE32(UChar32 &c, UErrorCode & ) {
    c = iter.next(&iter);
    if(c < 0) {
...
UChar32
UIterCollationIterator::nextCodePoint(UErrorCode & ) {
    return uiter_next32(&iter);
}

UChar32
UIterCollationIterator::previousCodePoint(UErrorCode & ) {
    return uiter_previous32(&iter);
}

void
UIterCollationIterator::forwardNumCodePoints(int32_t num, UErrorCode & ) {
...
void
UIterCollationIterator::backwardNumCodePoints(int32_t num, UErrorCode & ) {
