...
uint32_t
UTF16CollationIterator::handleNextCE32(UChar32 &c, UErrorCode & ) {
    if(pos == limit) {
        c = U_SENTINEL;
...
UChar32
UTF16CollationIterator::nextCodePoint(UErrorCode & ) {
    if(pos == limit) {
        return U_SENTINEL;
    }
    UChar32 c = *pos;
...
UChar32
UTF16CollationIterator::previousCodePoint(UErrorCode & ) {
    if(pos == start) {
        return U_SENTINEL;
    }
    UChar32 c = *--pos;
...
void
UTF16CollationIterator::forwardNumCodePoints(int32_t num, UErrorCode & ) {
...
void
UTF16CollationIterator::backwardNumCodePoints(int32_t num, UErrorCode & ) {
