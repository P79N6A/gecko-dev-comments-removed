    ...
    )
{
	...
	rv = SSL_ResetHandshake(ssl_sock,  1);
	if (rv != SECSuccess) {
		       ...
		       PRFileDesc *model_sock, CERTCertificate *cert)
{
    ...
    nextUpdate = now + 60*60*24 * PR_USEC_PER_SEC; 
