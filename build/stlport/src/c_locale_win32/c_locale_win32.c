...
_Locale_time_t* _Locale_time_create(const char * name, _Locale_lcid_t* lc_hint, int *__err_code) {
  ...
  for (month = LOCALE_SMONTHNAME1; month <= LOCALE_SMONTHNAME12; ++month) { 
  ...
  length = strlen(ltime->date_format) + strlen(ltime->time_format) + 1  + 1 ;
  ...
  length = strlen(ltime->long_date_format) + strlen(ltime->time_format) + 1  + 1 ;
